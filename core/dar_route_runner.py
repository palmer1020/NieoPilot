...
# 我方精灵名称列表（不需要识别这些）
MY_PET_NAMES = {"艾斯菲格", "闪光艾菲亚", "机塔", "艾菲德斯"}
...
BATTLE_OUT_BR_GRAY_BLUE_SPLIT = 72  # 出战 b−r：灰≈30、蓝≈145
BATTLE_SKILL4_RB_RED_MIN = 40  # 技能四 r−b：红偏大、蓝白为负（旧尼奥/稀有战后背包扫描用）
# Pick「机塔 vs 艾菲德斯」：勿与上式混用；见 `_pickmode_classify_skill4_rgb`（b−r / r−b 双轴）。
PICKMODE_SKILL4_RB_JITA_MIN = 26
PICKMODE_SKILL4_BR_AIFEIDESI_MIN = 22

# 对战切换精灵：打开面板→首次点槽；每点槽后→读出战/技能四探针前；选槽完成→单击出战前
SWITCH_PANEL_OPEN_TO_FIRST_SLOT_SEC = 1.5
SWITCH_PRE_READ_OUT_PROBE_SEC = 1.5
SWITCH_PRE_DEPLOY_WAIT_SEC = 1.5

# Pick 背包三精灵（仓库取宠 + 背包出战探针）在全场景已统一为同一只组合：
#   飞行系倒数 → 波克尔；超能系倒数第 20 → 艾菲德斯（入战 ID 见下）；机塔 → 双属性→机械龙系取「精灵仓库.1」。
...
#     口语「吉他」多指机塔。
#   • 旧尼奥/稀有战后：_scan_pet_probes_to_identify_pets → _classify_bag_skill4_mean_rgb(…,"nieo"|稀有)：
#     红＝艾斯菲格，蓝白＝闪光艾菲亚或亚梅丝；与 Pick 的 jita/aifeidesi 判色与身份均不同，勿混。
# 机塔 / 艾菲德斯入战 pet id（并表到允许列表与 pattern「我方」）
PICKMODE_JITA_PET_ID = 1337
PICKMODE_AIFEIDESI_PET_ID = 1459
# 入战 pattern：「我方」三精灵（含艾斯菲格入战 ID 12、旧 162/312/418 与新机塔/艾菲德斯）
FIGHT_MY_PET_IDS = {12, 162, 312, 418, PICKMODE_JITA_PET_ID, PICKMODE_AIFEIDESI_PET_ID}

# Pick 仓库机塔段时序（秒）：点击「双属性」→ 等待 → 点击「机械龙系」→ 等待 → 读「精灵仓库.1」
PICKMODE_WAREHOUSE_DUAL_ATTR_TO_MECHANICAL_SEC = 0.8
PICKMODE_WAREHOUSE_MECHANICAL_TO_SLOT1_PROBE_SEC = 0.5
# to 后地图零门控通过 → run() 内打开精灵背包恢复前的固定等待
MAP_ZERO_TO_RUN_RECOVER_DELAY_SEC = 2.0

# 入战收集pet ID的允许列表（超出则终端异常提醒）
NIEO_ALLOWED_PET_IDS = {
    1100000,
    10,
    12,
    16,
    ...
    416,
    418,
    PICKMODE_JITA_PET_ID,
    PICKMODE_AIFEIDESI_PET_ID,
}
# 尼奥/纯净能量野外入战提醒：至少命中其一（含 12、我方 312/418 或 Pick 机塔/艾菲德斯 id）。
# 与仓库 Pick 三精灵（波克尔/艾菲德斯/机塔）同一套入战 id 集合，仅本集合用于内核/探针「是否我方」校验。
NIEO_EXPECTED_IDS = {12, 312, 418, PICKMODE_JITA_PET_ID, PICKMODE_AIFEIDESI_PET_ID}
# 稀有模式公共允许 id：含敌方目标族 + 尼尔族 + 1100000 + Pick 机塔/艾菲德斯（波克尔为常见遇敌，由其它集合覆盖）
# 各模式再并 target_pet_ids；与「旧文案双塔三精灵/尼奥三精灵」无对应关系——仓库取宠已统一 Pick 见上。
...
MANTIS_PET_ID = 122
# 默认捕捉胶囊循环：超→超→特（与 UnifiedBattleFramework.DEFAULT_CAPSULE_CYCLE_TIERS 一致）
DEFAULT_CAPSULE_CYCLE_TIERS: Tuple[str, ...] = ("super", "super", "special")
# 历史上敌方 122「高超高特高超」六档；仅 CSV 回溯仍识别此常量。
...
DEFAULT_PROFILE_DUGULU = WildCaptureProfile(
    name="嘟咕噜(254)",
    route_hint="嘟咕噜",
    map_swf_id=323,  # 地图进入：``320to323.json``（to嘟咕噜 后落 320 → 点进 323；见 MAP_ENTRY_SCRIPT_STEMS）
    target_mp3_id=254,
    target_pet_id=254,
    delete_swf_ids=(252,),
    ab_cooldown_sec=40.0,
...
def wild_rare_map_gate_zero_literal_swf_ids(profile: WildCaptureProfile) -> FrozenSet[int]:
    """
    稀有野外：**刷新/to 后门控**允许的「地图零」内核字面 id（与 ``map_swf_id`` 地图一可能不同）。

    • 嘟咕噜 / 双塔 / 闪光皮皮 / 眼球：零与一为不同数字 → 非空集合；稳态只允许 ``profile.map_swf_id``，
      若在日志中又出现此处任一 id → 视为掉回零层，由 ``_detect_unexpected_map`` 触发刷新。
    • 小豆芽、螳螂达尔等：**零与一为同一字面（均为 11）** → 空集；``to`` 后/稳态均由严格 ``mid==11``，
      不单独维护「字面零集合」。
    """
...
class DarRouteRunner:
    ...
    # 纯净能量(资源)：技能四（及同计数节奏的尼尔/逃跑）每满 N 场恢复精灵一体力
    PEM_SKILL4_MAINTENANCE_EVERY_BATTLES = 30
    # map27 前缀「纯净能量一」：不参与扫描/标定的路线点（仍保留区域文件）
    PEM_ROUTE_SKIP_PURE_ENERGY_ONE = frozenset({"纯净能量一.8", "纯净能量一.9"})

    # 战后放回：双击精灵四后等待「选中四」高亮，超时触发重连四
    SELECT_PET_FOUR_TIMEOUT_S = 8.0
    ...
    def __init__(self, bot, regions: RegionStore, battle_runner, map_swf_id: int = 11):
        ...
        # Step2 内核 cursor：用于跳过取宠判定（须含进图 map 与三宠 swf，不能仅用屏蔽后 cursor）
        ...
        # 刷新 Step2：单击屏蔽后的 kernel cursor（保留作兜底）
        ...
        # 仅野外 `_execute_refresh_reconnect`：若 Step2 已 `_gate_map500001` 通过，下一进入重连核心时跳过一次全缓冲 500001 复验
        ...
        # 「非尼奥」北京时间段选用的稀有：`shuangta` | `mantis`（时段仍由 _detect_rotation_mode 决定）
        self._rotation_rare_slot: str = "shuangta"
        # 捕捉胶囊：默认超超特循环；Dashboard 可选仅超级/仅高级/仅特级单档备选
        self._non_mantis_capsule_tier: str = "cycle"
        # Pick 精灵模式 / 抗减命（由 Dashboard 任务字典注入）
        ...
        # 新重连 / Pick：用亚梅丝（仓库超能倒数3+20）替代机塔；不接 Dashboard，仅类内开关
        self._pickmode_yameisi_branch: bool = False
        # 稀有 / Pick 亚梅丝分支：背包技能四（rare）辨识度下的亚梅丝槽（「二」/「三」）
        ...
        # ✅ 测试模式：尼奥模式按地图强制切换尼尔家族流程（10→闪光艾菲亚=416，11→艾斯菲格=77）
        ...
        # 野外：log_battle_start 配对 id，战斗收尾或超时/刷新重连时 log_battle_end 或交由 log_reconnect 冲刷
        ...
        self._aisifeige_pos: Optional[str] = None  # 槽位「二/三」：旧流为艾斯菲格；Pick 流复用存艾菲德斯（见模块头注释）

        # ✅ 重连前清理缓存/tmp：_nieo_run_seen_nie_family 等仍用于其它自愈日志
    ...
    def run_nie_family_test(
        ...
        try:
            ...
            # Pick 背包：打开背包扫描二/三种机塔 / 艾菲德斯，供本场切换索引
            self._emit(
                "🔍 尼尔测试前置：打开背包扫描机塔 / 艾菲德斯槽位",
                "INFO",
            )
    ...
    def run_nieo_mode(
        ...
        2. 点击 地图/11to10.json（11→进入10）进入10号地图，等待10号地图+newNPC信号
        ...
        try:
            ...
            if pem:
                self._emit("⚡ [纯净能量(资源)] 模式启动（26/27 双图，全场技能四战胜）", "SYSTEM")
            else:
                self._emit("🌊 尼奥模式启动（10/11地图循环）", "SYSTEM")

            # ✅ 测试模式：保存到实例属性，供战斗分流时使用
            self._nieo_test_force_switch = bool(nieo_test_force_switch) and (not pem)
            # ✅ 同时保存到下划线前缀属性，方便重连后递归调用时读取
            ...
            if self._nieo_test_force_switch:
                self._emit(
                    "🧪 [尼奥测试模式] 已启用：10图所有遭遇切闪光艾菲亚（416路径），11图所有遭遇切艾斯菲格（77路径）",
                    "SYSTEM",
                )
            ...
            if pem:
                self._emit("💊 [纯净能量(资源)] 开场恢复精灵一 → 地图/26to27.json 首次进 27", "SYSTEM")
                self._is_recovering = True
                from core.dar_route_runner import DEFAULT_PROFILE_DUGULU
                temp_profile_pe = DEFAULT_PROFILE_DUGULU
                self._recover_pets(
                    use_foreground, stop_event, skip_return_storage=True, nie_family_id=None, profile=temp_profile_pe
                )
                self._is_recovering = False
                self._emit("🗺️ [纯净能量] 点击 地图/26to27.json（26→进入27）进入 27 号地图", "SYSTEM")
                if not self._execute_map_entry_script(27, use_foreground, stop_event):
                    self._emit("⚠️ 地图/26to27.json 执行失败，尝试继续", "WARN")
                if self.bot.stop_current or stop_event.is_set():
                    return
                if not self._wait_for_map_id(27, stop_event, timeout_s=30.0):
                    self._emit("⛔ [纯净能量] 等待地图27超时/停止", "WARN")
                    if stop_event.is_set() or getattr(self.bot, "stop_current", False):
                        self._emit("🛑 [纯净能量] 用户已请求停止", "INFO")
                        return
                    self._emit("🔄 [纯净能量-进图失败] 触发刷新重连", "WARN")
                    self._current_mode = "pure_energy"
                    self._execute_refresh_reconnect(
                        temp_profile_pe, use_foreground, stop_event, reason="纯净能量-进入地图失败"
                    )
                    if getattr(self, "_reconnect_scripts_executing", False):
                        mw = 300.0
                        ws = time.time()
                        while getattr(self, "_reconnect_scripts_executing", False) and (time.time() - ws) < mw:
                            time.sleep(0.5)
                    if getattr(self, "_should_restart_after_reconnect", False):
                        ...
                        self.run_pure_energy_resource_mode(
                            threading.Event(),
                            use_foreground,
                            nieo_pre_rotation_first=getattr(self, "_pem_pre_rotation_first", False),
                ...
                # 2. 点击 地图/11to10.json（11→进入10）进入10号地图，等待10号地图+newNPC信号
                self._emit("🗺️ 点击 地图/11to10.json（11→进入10）进入10号地图", "SYSTEM")
                if not self._execute_map_entry_script(10, use_foreground, stop_event):
                    self._emit("⚠️ 地图/11to10.json执行失败，尝试继续", "WARN")
                    ...
                    if not self._wait_for_map_id(10, stop_event, timeout_s=30.0):
                        ...
                        if getattr(self, "_should_restart_after_reconnect", False):
                            ...
                            self.run_nieo_mode(
                                ...
                                nieo_test_force_switch=getattr(self, "_nieo_test_force_switch", False),
                            )
                        return
                except Exception as e:
            ...
            while not stop_event.is_set() and not getattr(self.bot, "stop_current", False):
                ...
                if pem:
                    if current_map_id == 26:
                        current_prefix = "纯净能量二"  # 26 图：选点脚本「二」
                    else:
                ...
                # 所有图合计「跳过」至多 2：pem 在 27 图可手动不写 纯净能量一.8/.9，余量再按距入口剔最远
                ...
                scan_note = (
                    f"📐 [纯净能量] 手动跳过 {manual_route_skipped} + 按距剔除最远 {farthest_exclude}（合计≤2）→ {len(route_points_sorted)} 点扫描"
                    if pem
                    else "📐 扫描顺序已优化（基于距离，排除最远2个点，共7点参与扫描）"
                )
                ...
                # 标定基线（与排序「跳过配额」一致：手动 + 最远 合计≤2）
                baseline_note = (
                    f"📏 标定{current_prefix}（pem：手动跳过{manual_route_skipped} / 最远再剔{farthest_exclude}）"
                    if pem
                    else f"📏 标定{current_prefix}的7点基线（排除最远2个点）"
                )
                ...
                # 获取参考点（用于距离计算）；pem 与尼奥相同：取向另一张图的 to二/to一端点
                if pem:
                    if current_map_id == 26:
                        reference_key = "纯净能量二.to一"
                    else:
                        reference_key = "纯净能量一.to二"
                elif current_map_id == 10:  # 尼奥一
                ...
                if selected_key and selected_reg:
                    ...
                    # ✅ 尼奥模式：入战检测到的独特ID应包含 NIEO_EXPECTED_IDS 中至少一个，否则终端提醒
                    if not (unique_ids & NIEO_EXPECTED_IDS):
                        self._emit(
                            "⚠️ [尼奥模式提醒] 入战检测到的独特ID="
                            + str(sorted(unique_ids))
                            + "，不包含预期的任一ID（12/312/418/1337/1459），请检查配置或探针",
                            "WARN",
                        )
                    ...
                    if self._nieo_test_force_switch:
                        # ✅ 测试模式：强制按地图分流走尼尔家族战斗（10图→416闪光艾菲亚 / 11图→77艾斯菲格）
                        forced_nie_id = 416 if current_map_id == 10 else 77
                        target_label = "闪光艾菲亚(416路径)" if forced_nie_id == 416 else "艾斯菲格(77路径)"
                        self._emit(
                        ...
                        # ✅ 只要遇到过尼尔家族（含“勾选不捕捉尼尔而逃跑”的情况），本 run 即视为“遇到过”
            ...
            if nieo_stats is not None:
                ...
                self._emit(f"   稀有捕捉：{nieo_stats['稀有捕捉']}", "INFO")
                self._emit(
                    f"   尼尔家族：{nieo_stats['尼尔家族']}（77：{nieo_stats['77捕捉']}，310：{nieo_stats['310捕捉']}，416：{nieo_stats['416捕捉']}）",
                    "INFO",
                )
        ...
        if should_restart:
            ...
            # 注意：重连脚本执行完成后，游戏已通过 to 脚本回到目标地图
            new_stop_event = threading.Event()
            if pem:
                self._emit("🔄 [纯净能量-重连重启] 重新启动纯净能量模式", "SYSTEM")
                self.run_pure_energy_resource_mode(
                    new_stop_event,
                    use_foreground,
                    nieo_pre_rotation_first=getattr(self, "_pem_pre_rotation_first", False),
    ...
    def _configured_nieo_switch_map(
        ...
        """自定义三图尼奥：B↔C 切换，返回新 map id。"""
        if current_map_id == profile.map_b_id:
            stem = profile.transition_b_to_c
            target = profile.map_c_id
            label = f"{profile.prefix_b}.toC"
        elif current_map_id == profile.map_c_id:
            stem = profile.transition_c_to_b
            target = profile.map_b_id
            label = f"{profile.prefix_c}.toB"
        else:
            self._emit(f"⚠️ [自定义尼奥] 未知当前地图 {current_map_id}，无法切换", "WARN")
            return None

        reg = self.regions.get(label)
        if reg:
            self._emit(f"🗺️ [自定义尼奥] 点击 {label} → 地图 {target}", "SYSTEM")
            self._click_region(reg, use_foreground)
        else:
            self._emit(f"🗺️ [自定义尼奥] 点击 地图/{stem}.json → 地图 {target}", "SYSTEM")
            if not self._execute_map_entry_script_by_stem(stem, use_foreground, stop_event):
                return None
        if self._wait_for_map_id(target, stop_event, timeout_s=45.0):
            return target
        return None
    ...
    def run_configured_nieo_mode(
        ...
        try:
            ...
            self._emit(
                f"🌊 [自定义尼奥] {profile.name} 启动（A={profile.map_a_id} B={profile.map_b_id} C={profile.map_c_id}）",
                "SYSTEM",
            )
            ...
            self._emit(f"💊 [自定义尼奥] 在地图 {profile.map_a_id} 执行恢复流程", "SYSTEM")
            self._is_recovering = True
            self._recover_pets(
                use_foreground, stop_event, skip_return_storage=True,
                nie_family_id=None, profile=temp_profile,
            )
            self._is_recovering = False

            self._emit(
                f"🗺️ [自定义尼奥] 点击 地图/{profile.entry_stem}.json 进入地图 {profile.map_b_id}",
                "SYSTEM",
            )
            if not self._execute_map_entry_script_by_stem(profile.entry_stem, use_foreground, stop_event):
                self._emit("⚠️ [自定义尼奥] 进图脚本执行失败", "WARN")
            if not self._wait_for_map_id(profile.map_b_id, stop_event, timeout_s=30.0):
                self._emit("⛔ [自定义尼奥] 等待地图 B 超时", "WARN")
                return
            ...
            while not stop_event.is_set() and not getattr(self.bot, "stop_current", False):
                ...
                if current_map_id == profile.map_b_id:
                    ...
                    self._emit(f"❌ [自定义尼奥] 当前地图 {current_map_id} 不在 B/C 中", "ERROR")
                    break

                self._emit(
                    f"🗺️ [自定义尼奥] 地图 {current_map_id} 前缀 {current_prefix} 操作={action}",
                    "INFO",
                )
                ...
                route_points_sorted = self._sort_route_points_by_distance(
                    route_points, current_prefix, current_map_id, reg_z, farthest_exclude_count=2,
                )

                self._emit(f"🧭 [自定义尼奥] 点击 {current_prefix}.Z", "SYSTEM")
                self._sleep_abortable(stop_event, 0.3)
                self._click_region_twice(reg_z, use_foreground, gap=0.06)
                self._current_pos = self._region_center(reg_z)
                self._sleep_abortable(stop_event, 1.0)
                ...
                if current_map_id == profile.map_b_id:
                    ref_key = f"{profile.prefix_b}.toC"
                else:
                    ref_key = f"{profile.prefix_c}.toB"
                reference_reg = self.regions.get(ref_key) or reg_z
                reference_center = self._region_center(reference_reg)
                ...
                while (time.time() - scan_start) < max_scan:
                    if stop_event.is_set() or getattr(self.bot, "stop_current", False):
                        break
                    for key, reg in route_points_sorted:
                        ...
                        th = self._threshold.get(key, 9999.0)
                        if diff >= th:
                            pc = self._region_center(reg)
                            dist = self._dist2(pc, reference_center)
                            if selected_reg is None or dist < self._dist2(
                                self._region_center(selected_reg), reference_center
                            ):
                                selected_key, selected_reg = key, reg
                    if selected_reg is not None:
                        break
                    time.sleep(0.1)

                if selected_reg is None:
                    self._emit("⏱️ [自定义尼奥] 扫描超时，切图重试", "WARN")
                    nxt = self._configured_nieo_switch_map(
                        profile, current_map_id, use_foreground, stop_event
                    )
                    if nxt is None:
                ...
                self._emit(f"⚔️ [自定义尼奥] 点击变化点 {selected_key}", "SYSTEM")
                tx, ty = self._region_center(selected_reg)
                entry_result = self._nieo_click_until_battle(
                    tx, ty, selected_key, use_foreground, stop_event
                )
                if entry_result is None:
                    nxt = self._configured_nieo_switch_map(
                        profile, current_map_id, use_foreground, stop_event
                    )
                    current_map_id = nxt if nxt is not None else current_map_id
                    continue

                tx, ty, collected_pet_ids = entry_result

                if action == "defeat":
                    nxt = self._pem_after_change_skill4_defeat(
                        current_map_id=current_map_id,
                        current_prefix=current_prefix,
                        use_foreground=use_foreground,
                        stop_event=stop_event,
                    ...
                    nxt = self._configured_nieo_capture_after_change(
                        current_map_id=current_map_id,
                        current_prefix=current_prefix,
                        use_foreground=use_foreground,
                        stop_event=stop_event,
            ...
            self._emit(f"✅ [自定义尼奥] {profile.name} 结束", "SUCCESS")
        except Exception as e:
    ...
    def _nieo_click_until_battle(
        ...
        """持续点击直到 PetItem；返回 (tx, ty, collected_pet_ids) 或 None。"""
        from core.logger import fetch_kernel_since, kernel_cursor
            ...
            try:
                ...
                if isinstance(lines, list):
                    for line in lines:
                        line_str = str(line)
                        if line_matches(RE_PETITEM, line_str):
                            ids = self._collect_fight_pet_ids(
                                timeout=2.0, collect_window=0.4, stop_event=stop_event
                            )
                            collected_pet_ids = ids if ids else None
                            return tx, ty, collected_pet_ids
                    current_cursor = kernel_cursor()
            except Exception:
                pass
            time.sleep(0.05)
    ...
    def run_pure_energy_resource_mode(
        ...
        """
        纯净能量(资源)：重连 to纯净能量 → map26；首开进 27；之后 26↔27 与尼奥 10↔11 同构。
        普通战技能四战胜；遇尼尔家族（77/310/416）走与尼奥相同的技能一→切换→捕捉→战后恢复入仓；
        勾选「不捕捉尼尔」时遇 77 首回合逃跑。每满 ``PEM_SKILL4_MAINTENANCE_EVERY_BATTLES`` 场（当前 30）额外恢复精灵一。
        （前置重连由 bot_thread；`skip_nie_77` 仅程序内 API，Dashboard 不传 True。）
        """
        self._pem_pre_rotation_first = bool(nieo_pre_rotation_first)
        try:
            self.run_nieo_mode(stop_event, use_foreground, pem=True, skip_nie_77=skip_nie_77)
        finally:
        ...
        try:
            ...
            if is_rotation_mode and next_switch_time:
                ...
                if is_test_mode:
                    self._test_mode_start_time = time.time()  # ✅ 立即记录开始时间，而不是等到首次检测
                    self._emit(
                        f"🧪 [轮换模式-测试] {prof.name} 野外模式启动，记录开始时间（将在{self.ROTATION_RECONNECT_INTERVAL_MINUTES_SHUANGTA}分钟后切换）",
                        "INFO",
                    )
            ...
            # swf 删除/补齐在 bot_thread 任务入口（点击启动后、进入本 run 前）执行
            ...
            # ✅ profile 相关 swf 删除由 bot_thread 在启动野外任务入口先行调用 _check_and_delete_swf_files
            ...
            if profile.name == "闪光皮皮(164)":
                ...
                # 2) 执行地图进入脚本（切换到10号地图）
                self._emit(
                    "🗺️ [闪光皮皮] 执行地图进入脚本：地图\\11to10.json（11→进入10，切换到10号地图）",
                    "SYSTEM",
                )
                if not self._execute_map_entry_script(10, use_foreground, stop_event):
                    self._emit("⚠️ [闪光皮皮] 地图/11to10.json执行失败，尝试继续", "WARN")
            ...
            # ✅ 眼球：恢复后若已在 **地图一 60** 则跳过 61/61to60；若为 **61/62** 则点进 60；其它回退 61to60
                ...
                if latest_m == 60:
                    self._emit(
                        "ℹ️ [眼球] 内核已在地图一 map=60，跳过 61to60/62to60，直接进入狩猎稳态",
                        "INFO",
                    )
                    entry_stem = None
                elif latest_m == 62:
                    ...
                    if latest_m is not None:
                        self._emit(
                            f"⚠️ [眼球] 内核最新 map={latest_m}（非 61/62/60），回退执行 61to60",
                            "WARN",
                        )
                    entry_stem = "61to60"
                ...
                if entry_stem is not None:
                    self._emit(
                        f"🗺️ [眼球] 执行地图进入脚本：地图\\{entry_stem}.json（当前内核 map={latest_m} → 进入狩猎图 {profile.map_swf_id}）",
                        "SYSTEM",
                    )
                    if not self._execute_map_entry_script_by_stem(entry_stem, use_foreground, stop_event):
                        self._emit(f"⚠️ [眼球] 地图/{entry_stem}.json 执行失败，尝试继续", "WARN")
                ...
                # 普通流程。
                # 达尔 map11（螳螂/小豆芽）：已到 map11 时跳过重复进图（见 _dal_map11_already_ready_for_wild_capture）。
                # 螳螂：与轮换前置一致，to螳螂 后直接执行 地图/10to11 + 等待就绪（不经由日志强验 map10）。
                # 小豆芽等：进图前仍走 skip_return_storage 恢复。
                dal11_ready = self._dal_map11_already_ready_for_wild_capture(profile)
                if dal11_ready:
                    self._emit(
                        "💊 [达尔路线] map11+newNpc 已就位：跳过云上开场恢复与重复进图（地图/10to11），避免在 11 上再跑开场恢复",
                        "SYSTEM",
                    )
                else:
                    mantis_dar11 = (
                    ...
                    if mantis_dar11:
                        self._emit("🛤️ [螳螂] 执行 to螳螂，随后直接进入 map11（与轮换前置重连一致）", "SYSTEM")
                    ...
                    self._emit(f"🗺️ 执行地图进入脚本：地图\\{stem}.json（目标 map {profile.map_swf_id}）", "SYSTEM")
            ...
            # 启动常态1AND1检测（所有野外稀有 run() 模式，含自定义）
            ...
            while not stop_event.is_set() and (not getattr(self.bot, "stop_current", False)):
                        ...
                        if self._pending_mutation is not None:
                            ...
                            if now - pending_ts <= 2.0:
                                ...
                                if result is None:
                                    ...
                                    reached = self._recover_ab_patrol_after_refresh_click_failed(
                                        reg_a, reg_b, route_points, profile, use_foreground, stop_event
                                    )
                                    armed_until_ts = 0.0
                                    next_move_is_b = reached == "A"
                                    next_move_at = time.time() + profile.ab_cooldown_sec
                                    continue
                                tx, ty, collected_pet_ids = result
                                self._current_pos = (tx, ty)
                ...
                if hit is None:
                    ...
                    # ✅ 轮换模式：武装窗口已过期且无突变 → 仅在 A/B 巡逻点时才重新激活时间检测窗口
                    if self._is_rotation_mode and (not test_mode and profile.require_mp3_before_trigger):
                        if now > armed_until_ts and armed_until_ts > 0.0:
                            self._try_reactivate_rotation_time_check_window(
                                reg_a, reg_b, reason="[轮换检测-双塔模式] 武装窗口已过期且无突变",
                            )
                ...
                if not has_mp3_armed:
                    ...
                    # ✅ 轮换模式：武装窗口已过期 → 仅在 A/B 巡逻点时才重新激活时间检测窗口
                    self._try_reactivate_rotation_time_check_window(
                        reg_a, reg_b, reason="[轮换检测-双塔模式] 武装窗口已过期",
                    )
            ...
            if should_restart:
                ...
                new_stop_event = threading.Event()
                self._emit(
                    f"🐞 [重连重启] 即将递归调用 run() 重启，profile={profile.name}, "
                    f"is_rotation_mode={self._is_rotation_mode}",
                    "DEBUG",
                )
                self.run(
                    new_stop_event,
    ...
    def _is_at_ab_patrol_point(
        ...
        """当前位置是否在 A 或 B 巡逻锚点附近。返回 (是否在巡逻点, 'A'|'B'|None)。"""
        if self._current_pos is None:
            return False, None
        ax, ay = self._region_center(reg_a)
        bx, by = self._region_center(reg_b)
    ...
    def _recover_ab_patrol_after_refresh_click_failed(
        ...
        """
        刷新点持续点击超时未入战：确认回到 A/B（非刷新点）后重新标定稳态，再恢复扫描。
        返回所到达的锚点名称 "A" 或 "B"。
        """
        self._emit("↩ 刷新点入战失败，确认离开刷新点回到 A/B 后恢复稳态", "WARN")
        if stop_event.is_set() or getattr(self.bot, "stop_current", False):
        ...
        if reached is None:
            ...
            self._emit(
                f"⚠️ 未能检测确认离开刷新点，兜底点击反向点{opposite_name}",
                "WARN",
            )
            self._click_region(opposite_reg, use_foreground)
            ...
            self._emit(f"✅ 已确认离开刷新点，到达{reached}点", "SUCCESS")

        self._emit("📏 重新标定稳态（确认回到 A/B 后，检测所有9个点）", "SYSTEM")
        ...
        if self._is_rotation_mode and not self._rotation_time_check_window_active:
            self._rotation_time_check_window_active = True
            self._last_rotation_time_check = 0.0
            self._emit("🔄 [轮换检测] 已回巡逻点并恢复稳态，重新激活时间检测窗口", "INFO")
    ...
    def _check_and_delete_swf_files(self, profile: WildCaptureProfile):
        """
        检查并删除指定的 swf 文件（根据 profile.delete_swf_ids）。
        由 bot_thread 在点击启动野外/轮换稀有段前调用。

        每个 id 在 pet/swf 与 groupFightResource/pet 两处均检查；存在则先备份 OG 再删。
        """
        try:
            ...
            if not files_to_delete:
                self._emit(f"ℹ️ [{profile.name}] 未配置 delete_swf_ids，跳过 swf 删除", "INFO")
                return
            ...
            for pet_id in sorted(files_to_delete):
                ...
                for folder, ensure_og, label in delete_targets:
                    if not os.path.isdir(folder):
                        self._emit(f"⚠️ {label} 目录不存在: {folder}，跳过 {name}", "WARN")
                        continue
                    target_file = os.path.join(folder, name)
                    if not os.path.exists(target_file):
                        self._emit(f"ℹ️ [{profile.name}] {label} 无 {name}，跳过", "DEBUG")
                        continue
                    try:
                        ensure_og(target_file)
                        os.remove(target_file)
                        self._emit(f"🗑️ 已删除 {label}: {name}", "INFO")
                    except Exception as e:
    ...
    def _try_delete_game_tmp_dir(self, *, log_tag: str = "") -> None:
        ...
        try:
            ...
            self._emit(f"⚠️ [{log_tag}] 无法读取 config.GAME_PATH / shutil：{e}", "WARN")
            return
        try:
            game_root = os.path.dirname(os.path.abspath(GAME_PATH))
            tmp_dir = os.path.join(game_root, "tmp")
            if not os.path.isdir(tmp_dir):
                self._emit(f"ℹ️ [{log_tag}] 未找到 tmp，跳过删除：{tmp_dir}", "INFO")
                return
            shutil.rmtree(tmp_dir)
            self._emit(f"✅ [{log_tag}] 已删除 tmp：{tmp_dir}", "SUCCESS")
        except Exception as e:
            self._emit(f"⚠️ [{log_tag}] 删除 tmp 失败（可能被占用）：{e}", "WARN")
    
    def _check_and_fill_missing_swf_files(self):
        """
        从模板复制并补齐缺失占位 swf。由 bot_thread 在启动尼奥/轮换/训练室/野外（螳螂测试）等任务入口调用。
        - 检查指定文件夹中的swf文件
        ...
        try:
            ...
            # ✅ 各野外稀有「删占位」并集子集：含眼球所需的 207（与 _check_and_delete 眼球条目一致）
    ...
    def _rotation_calendar_segment_for_swf_sync(self) -> str:
        """
        与 ``_detect_rotation_mode`` 的日历段对齐：尼奥为 ``nieo``；双塔与螳螂等非尼奥稀有均为 ``shuangta``。
        供轮换期间 ``_execute_refresh_reconnect`` 等路径按「当前引擎模式」推导应用哪套磁盘 SWF 策略。
        """
    ...
    def _sync_swf_for_rotation_segment(
        ...
        *,
        ...
        """
        ``calendar_mode``: ``nieo`` | ``shuangta``（与 ``_detect_rotation_mode`` 一致；螳螂同属 shuangta 段）。
        - 尼奥段：仅 ``_check_and_fill_missing_swf_files``；
        - 稀有段：补齐后按 ``_rotation_rare_slot`` 选双塔或螳螂 profile 再 ``_check_and_delete_swf_files``。
        """
        self._check_and_fill_missing_swf_files()
        if calendar_mode == "nieo":
            self._emit(f"📦 [{log_tag}] SWF：尼奥段补齐并集（不删野外占位）", "INFO")
            return
        rs = getattr(self, "_rotation_rare_slot", "shuangta") or "shuangta"
        prof = self._resolve_rotation_rare_profile(rs)
        self._check_and_delete_swf_files(prof)
        self._emit(f"📦 [{log_tag}] SWF：稀有段 {prof.name} 已按 profile 删占位", "INFO")
    ...
    def _save_client_screenshot(
        ...
        *,
        ...
        """
        通用：对整个 client 截图；默认保存到 screenshots/client/。
        若传入 category（如 rotation / reconnect_four / round_timeout），则保存到 screenshots/client/<category>/。
        """
        try:
    ...
    def _maybe_save_rotation_pre_switch_screenshot(self, tag_suffix: str) -> None:
        """轮换模式（_is_rotation_mode）下，在打开切换面板、点选精灵出战前，保存整幅 client 截图到 screenshots/client/rotation/。"""
        if not getattr(self, "_is_rotation_mode", False):
            return
        try:
            path = self._save_client_screenshot(
                f"轮换_切换精灵出战前_{tag_suffix}", category="rotation"
            )
            if path:
                self._emit(f"📸 [轮换] 切换前已保存client截图：{path}", "INFO")
        except Exception as e:
            self._emit(f"⚠️ [轮换] 切换前截图失败: {e}", "WARN")

    def _scan_pickmode_bag_slots_skill4(
        self, use_foreground: bool, stop_event: threading.Event
    ) -> None:
        """
        Pick 背包：依次双击精灵二、三并读技能四探针（pickmode 分类），
        更新 _jita_pos / _aisifeige_pos（后者在 Pick 流存艾菲德斯槽，见 `_pickmode_classify_skill4_rgb`）。
        """
        jita_p: Optional[str] = None
        ad_p: Optional[str] = None
        self._emit(
            "🔍 [背包探针-Pick] 扫描精灵二、三（技能四：b−r 蓝侧=艾菲德斯，r−b 红侧=机塔；见 _pickmode_classify_skill4_rgb）",
            "INFO",
        )
        for pos_cn in ("二", "三"):
            self._click_bag_pet_slot_double(pos_cn, use_foreground)
            self._sleep_abortable(stop_event, 0.55)
            k = self._read_bag_skill4_probe_kind("pickmode")
            self._emit(f"📋 [背包探针-Pick] 精灵{pos_cn} → {k}", "INFO")
            if k == "jita":
                jita_p = pos_cn
            elif k == "aifeidesi":
                ad_p = pos_cn
        if jita_p:
            self._jita_pos = jita_p
        if ad_p:
            self._aisifeige_pos = ad_p
        if jita_p and ad_p:
            self._emit(
                f"✅ [背包探针-Pick] 机塔=精灵{jita_p}，艾菲德斯=精灵{ad_p}",
                "SUCCESS",
            )
        elif jita_p or ad_p:
            self._emit(
                f"⚠️ [背包探针-Pick] 仅辨明部分槽位：机塔={jita_p}，艾菲德斯={ad_p}",
                "WARN",
            )
        else:
            self._emit("❌ [背包探针-Pick] 未能识别机塔/艾菲德斯槽", "WARN")
    ...
    def _scan_pickmode_bag_slots_yameisi_branch(
        ...
        """
        Pick「亚梅丝替代机塔」分支：依次双击精灵二、三，
        同一帧读技能四均值：pickmode（艾菲德斯）+ rare（亚梅丝 / 艾斯菲格），
        写入 _yameisi_pos / _aisifeige_pos，并把 _jita_pos 设为与 _yameisi_pos 同槽
        （沿用稀有切换里「机塔槽优先序」字段，避免大范围改名）。
        """
        self._emit(
            "🔍 [背包探针-Pick·亚梅丝分支] 扫描精灵二、三（Pick 判艾菲德斯 + rare 判亚梅丝）",
            "INFO",
        )
        ym_p: Optional[str] = None
        ad_p: Optional[str] = None
        for pos_cn in ("二", "三"):
            self._click_bag_pet_slot_double(pos_cn, use_foreground)
            self._sleep_abortable(stop_event, 0.55)
            ...
            if rgb is None:
                self._emit(f"⚠️ [背包探针-Pick·亚梅丝] 精灵{pos_cn} 无法读技能四", "WARN")
                continue
            r, g, b = rgb
            pk = self._pickmode_classify_skill4_rgb(r, g, b)
            rz = self._classify_bag_skill4_mean_rgb(rgb, "rare")
            self._emit(
                f"📋 [背包探针-Pick·亚梅丝] 精灵{pos_cn} pickmode={pk}，rare={rz}",
                "INFO",
            )
            if pk == "aifeidesi":
                ad_p = pos_cn
            if rz == "yameisi":
                ym_p = pos_cn
        if ym_p:
            self._yameisi_pos = ym_p
            self._jita_pos = ym_p
        if ad_p:
            self._aisifeige_pos = ad_p
        if ym_p and ad_p:
            self._emit(
                f"✅ [背包探针-Pick·亚梅丝] 亚梅丝=精灵{ym_p}，艾菲德斯=精灵{ad_p}",
                "SUCCESS",
            )
        elif ym_p or ad_p:
            self._emit(
                f"⚠️ [背包探针-Pick·亚梅丝] 仅部分辨明：亚梅丝={ym_p}，艾菲德斯={ad_p}",
                "WARN",
            )
        else:
            self._emit("❌ [背包探针-Pick·亚梅丝] 未能识别亚梅丝/艾菲德斯槽", "WARN")

    def _scan_pickmode_bag_slots_respecting_yameisi_branch(
        self, use_foreground: bool, stop_event: threading.Event
    ) -> None:
        """Pick 背包二/三槽位扫描：机塔流用 pickmode；亚梅丝分支用 rare+pickmode 组合（见上）。"""
        if getattr(self, "_pickmode_yameisi_branch", False):
            self._scan_pickmode_bag_slots_yameisi_branch(use_foreground, stop_event)
        else:
            self._scan_pickmode_bag_slots_skill4(use_foreground, stop_event)
    ...
    def _recover_pets(self, use_foreground: bool, stop_event: threading.Event, skip_return_storage: bool = False, nie_family_id: Optional[int] = None, profile: Optional[WildCaptureProfile] = None):
        ...
        2. 如果skip_return_storage为False（捕捉成功后）：
           - 打开背包后扫描清空精灵一探针(#FF9901)就绪后前置：快速双击精灵四，超时内选中四高亮则继续，否则重连四
        ...
        - Pick：出战为艾菲德斯 → 恢复艾菲德斯槽；战后扫描机塔+艾菲德斯。
        
        尼尔家族77/310：
        - 尼奥(run_nieo)：恢复艾斯菲格槽；战后按模式扫描（尼奥=闪光+艾斯菲格，稀有=亚梅丝+艾斯菲格）。
        - Pick：出战为艾菲德斯 → 恢复艾菲德斯槽；战后扫描机塔+艾菲德斯。
        
        稀有无尼尔：
        - 经典稀有：恢复快出战亚梅丝；Pick 且本场切换机塔则恢复机塔槽；否则默认精灵一。
        ...
        try:
            ...
            # 打开精灵背包后扫描清空精灵一探针(#FF9901)就绪，然后前置：双击精灵四+选中四检测（超时未选中则重连四）
            self._wait_pet_bag_ui_ready_after_open(stop_event)
            ...
            if not skip_return_storage:
                ...
                if nie_family_id == 416:
                    hint_ad = (
                        self._aisifeige_pos
                        if self._aisifeige_pos in ("二", "三")
                        else "二"
                    )
                    recover_pos = self._bag_ensure_slot_for_recover_kind(
                        "aifeidesi",
                        "pickmode",
                        use_foreground,
                        hint_pos=hint_ad,
                    ...
                    self._emit(
                        f"🔄 [416尼尔] 恢复艾菲德斯（精灵{recover_pos}，本场出战槽）",
                        "INFO",
                    )
                    
                    self._sleep_abortable(stop_event, 0.3)
                    self._emit(f"💊 点击精灵恢复（当前选中精灵{recover_pos}）", "INFO")
                    ...
                    self._emit(
                        f"🔄 [77/310尼尔] 恢复艾菲德斯（精灵{recover_pos}，本场出战槽）",
                        "INFO",
                    )
                    ...
                    # 无尼尔：第二场出战机塔/亚梅丝分支则恢复对应槽，否则恢复精灵一
                    if getattr(self, "_pickmode_battle_on_jita", False):
                        if getattr(self, "_pickmode_yameisi_branch", False):
                            hint_y = (
                                self._yameisi_pos
                                if self._yameisi_pos in ("二", "三")
                                else (
                                    self._jita_pos
                                    if self._jita_pos in ("二", "三")
                                    else "二"
                                )
                            )
                            recover_pos = self._bag_ensure_slot_for_recover_kind(
                            ...
                            self._emit(
                                f"🔄 [野遇稀有·Pick亚梅丝] 恢复亚梅丝（精灵{recover_pos}）",
                                "INFO",
                            )
                        else:
                            hint_j = (
                            ...
                            self._emit(
                                f"🔄 [野遇稀有] 恢复机塔（精灵{recover_pos}，本场出战槽）",
                                "INFO",
                            )
                    elif profile and hasattr(self, '_yameisi_pos') and self._yameisi_pos:
                        hint_y = (
                            self._yameisi_pos
                            if self._yameisi_pos in ("二", "三")
                            else "二"
                        ...
                        self._emit(
                            f"🔄 [稀有精灵-出战] 恢复亚梅丝（精灵{recover_pos}，技能四探针选槽）",
                            "INFO",
                        )
                    else:
                        recover_pos = "一"
                        self._emit(f"🔄 [正常] 恢复精灵一", "INFO")
                        self._emit(f"🐾 双击精灵一（准备恢复）", "INFO")
                    ...
                    if recover_pos != "一":
                        self._emit(f"🐾 已选中精灵{recover_pos}（技能四探针对齐），准备恢复", "INFO")
                ...
                # 第一次进图恢复：补齐机塔 / 艾菲德斯槽缓存（Pick 背包）
                self._emit(
                    "🔍 在1AND1消失后，扫描精灵二、三（机塔 / 艾菲德斯）（第0次）",
                    "INFO",
                )
                self._scan_pickmode_bag_slots_respecting_yameisi_branch(
                    use_foreground, stop_event
            ...
            # ✅ 检查重连条件：硬线 OR 最小值×PETSWF_TO_PETITEM_MIN_MULTIPLIER（不再检查40次战斗条件）
            # 注意：只有在没有因为精灵四放回仓库问题重连的情况下才执行
            if not getattr(self, '_reconnect_reason_capture_verify_four', False):
                ...
                if not should_reconnect:
                    if (self._petswf_to_petitem_current_duration is not None and 
                        ...
                        threshold = self._petswf_to_petitem_min_duration * _pm
                        # ✅ 修复：添加调试日志，确保逻辑正确，并改进日志输出显示最小值和阈值
                        self._emit(f"🔍 [重连检查] 当前时间={self._petswf_to_petitem_current_duration:.3f}s, 最小值={self._petswf_to_petitem_min_duration:.3f}s, 阈值(×{_pm:g})={threshold:.3f}s", "DEBUG")
                        if self._petswf_to_petitem_current_duration >= threshold:
                            should_reconnect = True
                            reconnect_reason = f"petswf到PetItem时间差 ({self._petswf_to_petitem_current_duration:.3f}s) 超过最小值的{_pm:g}倍 (最小值={self._petswf_to_petitem_min_duration:.3f}s, 阈值={threshold:.3f}s)"
    ...
    def _execute_map_entry_script(
        ...
        """
        执行地图进入脚本：读取 ``regions/地图/{stem}.json``；目标 ``map_swf_id`` 通过 ``MAP_ENTRY_SCRIPT_STEMS`` 映射 ``stem``
        （通常为「起点图→目标图」语义），否则 ``stem=str(map_swf_id)``。
        
        Args:
            map_swf_id: 目标地图 swf id（脚本语义为「从邻图点击进入该图」）
            use_foreground: 是否前台执行
            stop_event: 停止事件
            profile: 可选，自定义模式提供 map_entry_stem
    ...
    def _execute_map_entry_script_by_stem(
        ...
        """
        按 ``regions/地图/{stem}.json`` 执行（与 ``_execute_map_entry_script`` 同源）。
        眼球模式按内核当前图选用 ``61to60`` / ``62to60``。
        """
        try:
            import json
            ...
            if "key" in data:
                ...
                self._emit(f"🗺️ 点击地图进入点：{region_key}（{stem}）", "SYSTEM")
                try:
    ...
    def _dal_map11_already_ready_for_wild_capture(self, profile: WildCaptureProfile) -> bool:
        """
        达尔挂机图（map_swf_id=11）：to 脚本后通常在云上(map10)再点 11.json，
        云上「开场恢复」只应在进 11 前执行。若前置轮换重连 / 重连已落定 map11+newNpc，
        则不应在 11 再跑一遍开场恢复，也无需重复执行 地图/10to11.json。
        """
        try:
    ...
    def _kernel_map_matches_profile_expectation(
        ...
        """
        内核日志 map 比对：**仅用于刷新/to 后的「地图零」门控**（与字面集合一致）。

        若 ``wild_rare_map_gate_zero_literal_swf_ids(profile)`` 非空：**mid 须属于该集合**（嘟咕噜 320、双塔 315、
        闪光 11、眼球 61/62）。否则 **mid == expected_map_id**（小豆芽等零一同一字面）。
        稳态「必须停在地图一」见 ``_detect_unexpected_map``。
        """
    ...
    def _detect_unexpected_map(
        ...
        """
        稳态扫描：期望 **始终** 为 ``expected_map_id``（地图一）。若该模式存在独立「地图零」字面 id（与一不同），
        则日志中出现 **任何非地图一** 的 map 行（含掉回零层）均视为不一致。
        """
    ...
    def _handle_battle_trigger(
        ...
        # ✅ 螳螂模式：仅敌方含野怪 122（对手螳螂）时第一回合无敌；其余走技能一单档胶囊（由 _non_mantis_capsule_tier 驱动）
        ...
        if not pet_ids and is_mantis_mode:
            ...
            if retry_pet_ids:
                ...
                self._emit(
                    "⚠️ [螳螂模式] 无法判定敌方是否含 122，第一回合不使用无敌胶囊（按默认技能一单档胶囊）",
                    "WARN",
                )
        ...
        if pet_ids:
            ...
            if nie_family_ids:
                ...
                if is_mantis_mode:
                    if MANTIS_PET_ID in unique_ids:
                        ...
                        self._emit(
                            "🛡️ [螳螂模式] 检测到敌方包含 122，第一回合使用无敌精灵胶囊",
                            "SYSTEM",
                        )
                    else:
                        self._emit(
                            "ℹ️ [螳螂模式] 敌方不含 122，第一回合不按无敌胶囊（与普通野外一致开局）",
                            "INFO",
                        )
                else:
            ...
            # 螳螂模式且仍无 IDs：不按无敌胶囊（已在上面给出 WARN）
        ...
        if self._unified_framework and self._wild_adapter:
            ...
            def action_callback(round_idx: int) -> str:
                ...
                if test_mode:
                    ...
                    if round_idx == 1:
                        if invincible_first_round:
                            return "capsule"  # 仅螳螂遇 122：第一回合无敌胶囊
                        else:
                            return "skill"
                    return "capsule"
        ...
        # CSV battle log：先录入 start 再行尾补录 end（paired battle_instance_id）
        try:
    ...
    def _do_battle_capture(
        ...
        # ✅ 非统一框架野外时：捕捉前双击切换对战/捕捉面板。统一框架野外时 Stage2 已打完第一回合，此处跳过以免与出招竞态；投胶囊时由 _execute_action 再切面板。
        ...
        if self._unified_framework and self._wild_adapter:
            ...
            def action_callback(round_idx: int) -> str:
                ...
                # ✅ 非螳螂野外：视为稀有线（双塔/嘟咕噜/眼球等），含 77/310/416 的尼尔战处理
                # 注意：77/310 在稀有线走亚梅丝+艾斯菲格探针，仅在 run_nieo 时 _current_mode=="nieo" 走尼奥探针
                ...
                if nie_family_id is not None:
                    ...
                    if is_rare_mode and nie_family_id == 416:
                        if round_idx == 1:
                            ...
                            # 第二回合开始直接扔高级胶囊（不切换）
                            return "capsule"
                    # 稀有线遇 77/310：亚梅丝VS艾斯菲格探针，出战艾斯菲格（非 run_nieo 时）
                    elif (
                        is_rare_mode
                    ...
                    if has_target_pet or (not has_target_pet and nie_family_id is None):
                        ...
                        if round_idx == 1:
                            ...
                            if getattr(self, "_pick_pet_mode", False) and not (
                                ...
                                self._skill1_cycle_phase = None
                                self._emit(
                                    f"🎯 [稀有精灵·Pick] 第{round_idx}回合：使用技能一（第1次）",
                                    "INFO",
                                )
                                return "skill"
                            return "skill2"
                            ...
                            if (
                                ...
                                self._skill1_cycle_phase = None
                                self._emit(
                                    f"🎯 [稀有精灵] 第{round_idx}回合：使用技能一（第{self._skill1_count_rare}次）",
                                    "INFO",
                                )
                                return "skill"
                        ...
                        if (
                            ...
                            # 第六回合及以后；或 Pick 交替时第五回合起（首记技能一已在第4回合）
                            
                            # 如果技能一达到上限，之后只投胶囊循环（档位由框架单档覆盖）
    ...
    def _get_rare_mode_allowed_ids(self, profile: "WildCaptureProfile") -> set:
        """稀有模式：base(12/166/197/418/77/310/416/1337/1459) + 各模式目标精灵"""
    ...
    def _classify_and_log_pet_ids(self, pet_ids: set, profile: WildCaptureProfile) -> None:
        ...
        # 定义类别
        MY_PETS = FIGHT_MY_PET_IDS  # 我方精灵（含 12、162/312/418 与 Pick 机塔/艾菲德斯；注意不是166）
        ...
        # ✅ 稀有模式（双塔/闪光皮皮/眼球/嘟咕噜）：允许列表检查 = base(12/166/197/418/77/310/416) + 各模式目标精灵
        profile_name_lower = profile.name.lower()
        if "双塔" in profile_name_lower:
            SHUANGTA_EXPECTED_IDS = {
                12,
                166,
                197,
                418,
            ...
            if not (unique_ids & SHUANGTA_EXPECTED_IDS):
                self._emit(
                    f"⚠️ [双塔模式提醒] 入战检测到的独特ID={sorted(unique_ids)}，"
                    f"不包含预期的任一ID（12/166/197/418/1337/1459），请检查配置或探针",
                    "WARN",
                )
    ...
    def _check_fight_pet_pattern(
        ...
        """
        # 定义三类精灵
        MY_PETS = FIGHT_MY_PET_IDS  # 第一类：我方精灵（含 12、162/312/418 与 Pick 机塔/艾菲德斯）
    ...
    def _pickmode_classify_skill4_rgb(self, r: int, g: int, b: int) -> str:
        """
        Pick 下「机塔 vs 艾菲德斯」技能四判色（背包与对战切换同式）：
        先判蓝侧艾菲德斯、再判红侧机塔；均不达标时比较 B−R 与 R−B 谁占优。
        """
        rb = r - b
        br = b - r
        ...
        if rb >= PICKMODE_SKILL4_RB_JITA_MIN:
            return "jita"
        return "aifeidesi" if br > rb else "jita"
    ...
    def _classify_bag_skill4_mean_rgb(
        ...
        """
        背包「精灵背包.技能四探针」均值分类。
        - mode == "pickmode"：机塔/艾菲德斯见 `_pickmode_classify_skill4_rgb`（双轴，非单用 r−b≥40）。
        - 否则：偏红→艾斯菲格；nieo 且偏蓝白→闪光艾菲亚；稀有且偏蓝白→亚梅丝。
        """
        r, g, b = rgb
        if mode == "pickmode":
    ...
    def _bag_select_slot_by_skill4_probe(
        self, wanted_kind: str, mode: str, use_foreground: bool
    ) -> Optional[str]:
        """依次双击精灵二、三并读技能四探针，返回首个与 wanted_kind 一致的槽位。"""
        for pos_cn in ("二", "三"):
            self._click_bag_pet_slot_double(pos_cn, use_foreground)
            time.sleep(0.5)
            k = self._read_bag_skill4_probe_kind(mode)
            self._emit(
                f"📋 [背包技能四探针] 选中精灵{pos_cn} → {k}",
                "DEBUG",
            )
            if k == wanted_kind:
                self._emit(
                    f"✅ [背包选槽] 精灵{pos_cn} 匹配目标 {wanted_kind}，用于恢复",
                    "SUCCESS",
                )
                return pos_cn
        return None
    ...
    def _bag_ensure_slot_for_recover_kind(
        ...
        """恢复前选中正确二/三槽：探针顺序试二、三；均不匹配则回退双击 hint（默认可「二」）。"""
        sel = self._bag_select_slot_by_skill4_probe(wanted_kind, mode, use_foreground)
        if sel is not None:
            return sel
        fb = hint_pos if hint_pos in ("二", "三") else "二"
        self._emit(
            f"⚠️ [背包恢复] 技能四探针未匹配 {wanted_kind}，回退双击精灵{fb}",
            "WARN",
        )
    ...
    def _detect_battle_switch_pet_type_color_probes(
        ...
        """
        对战切换：先读「出战探针」区分灰/蓝；偏灰则为波克尔系，不再读技能四。
        偏蓝则读「对战切换.技能四探针」：Pick 同式 `_pickmode_classify_skill4_rgb`（机塔 vs 艾菲德斯）。

        Returns:
            nieo: boker | jita | aifeidesi
            rare: flash_boker | jita | aifeidesi
            None: 区域缺失或无法判别
        """
        _ = use_foreground
        out_key = "对战切换.出战探针"
        sk_key = "对战切换.技能四探针"
        out_rgb = self._mean_rgb_for_region_key(out_key)
        if out_rgb is None:
            self._emit("⚠️ [对战切换色探针] 无法读取出战探针区域", "WARN")
            return None
        r, g, b = out_rgb
        br = b - r
        out_is_blue = br >= BATTLE_OUT_BR_GRAY_BLUE_SPLIT
        self._emit(
            f"📋 [对战切换色探针] 出战 RGB=({r},{g},{b}) B-R={br} → "
            f"{'偏蓝' if out_is_blue else '偏灰'}",
            "DEBUG",
        )
        if not out_is_blue:
            label = "波克尔" if mode == "nieo" else "闪光波克尔"
            self._emit(f"📋 [对战切换色探针] 出战偏灰 → {label}（跳过技能四探针）", "DEBUG")
            return "boker" if mode == "nieo" else "flash_boker"

        sk_rgb = self._mean_rgb_for_region_key(sk_key)
        if sk_rgb is None:
            self._emit("⚠️ [对战切换色探针] 出战偏蓝但无法读取技能四探针", "WARN")
            return None
        r4, g4, b4 = sk_rgb
        kind = self._pickmode_classify_skill4_rgb(r4, g4, b4)
        rb = r4 - b4
        br = b4 - r4
        self._emit(
            f"📋 [对战切换色探针] 技能四 RGB=({r4},{g4},{b4}) R−B={rb} B−R={br} → {kind}",
            "DEBUG",
        )
        if kind == "jita":
            self._emit("📋 [对战切换色探针] 技能四 → 机塔", "DEBUG")
        else:
            self._emit("📋 [对战切换色探针] 技能四 → 艾菲德斯", "DEBUG")
        return kind

    def _resolve_pet_type_at_current_switch_slot(
        self, use_foreground: bool, mode: str = "nieo"
    ) -> Optional[str]:
        """色探针优先，失败则回退四字/五字黄字探针。"""
    ...
    def _detect_switch_probe_pet_type(self, use_foreground: bool, mode: str = "nieo") -> Optional[str]:
        """
        检测当前选中精灵类型：优先「对战切换」出战/技能四 RGB；失败则回退旧尼奥/稀有切换探针距离法。
        
        尼奥模式返回值含 boker / aisifeige / flash_aifeia；稀有模式含 flash_boker / aisifeige / yameisi。
        """
        pt = self._detect_battle_switch_pet_type_color_probes(use_foreground, mode)
        if pt is not None:
            return pt
    ...
    def _switch_pet_for_nie_family(
        ...
        """
        根据尼尔家族ID使用智能切换（默认 Pick：出战艾菲德斯）。
        
        出战/技能四色探针（偏灰为波克尔不读技能四；偏蓝则技能四区分机塔 vs 艾菲德斯）；
        按槽位顺序检测，命中目标即出战。色探针失败回退四字/五字。
        ...
        try:
            if nie_family_id not in (77, 310, 416):
                ...
                return False
            # 全局 Pick：尼尔遭遇统一切艾菲德斯（与背包二/三技能四探针对齐）
            target_pet_type = "aifeidesi"
            self._emit("🎯 [尼尔家族] 出战艾菲德斯（Pick 背包）", "INFO")
            # 测试模式：使用旧逻辑（向后兼容）
            if test_mode:
            ...
            for pet_num in slot_scan_order:
                self._emit(f"🔍 [智能切换] 扫描精灵{pet_num}（色探针优先，出战灰则跳过技能四）", "INFO")
                sk = f"对战.切换精灵.切换精灵{idx_cn[pet_num]}"
                try:
                    self._click_region_twice(sk, use_foreground)
                except KeyError:
                    self._emit(f"⚠️ 找不到切换精灵{idx_cn[pet_num]}（{pet_num}）的区域，切换失败", "WARN")
                    return False
                time.sleep(SWITCH_PRE_READ_OUT_PROBE_SEC)
                self._current_switching_pet_num = pet_num
                pt = self._resolve_pet_type_at_current_switch_slot(use_foreground, "nieo")
                results[pet_num] = pt
                last_scanned = pet_num
                self._emit(f"📋 [智能切换] 精灵{pet_num} → {pt or '未知'}", "INFO")
                if pt == target_pet_type:
                    chosen_slot = pet_num
                    self._emit(
                        f"✅ [智能切换] 精灵{pet_num} 已匹配目标（{target_pet_type}），直接出战",
                        "SUCCESS",
                    )
                    break
            ...
            if chosen_slot is None:
                self._emit(
                    "⚠️ [智能切换] 未匹配到目标类型，回退假定精灵1为目标",
                    "WARN",
                )
                chosen_slot = 1

            if chosen_slot != last_scanned:
                sk = f"对战.切换精灵.切换精灵{idx_cn[chosen_slot]}"
                try:
                    self._click_region_twice(sk, use_foreground)
                except KeyError:
                    self._emit(f"⚠️ 找不到切换精灵{idx_cn[chosen_slot]}，切换失败", "WARN")
                    return False

            time.sleep(SWITCH_PRE_DEPLOY_WAIT_SEC)
            self._emit(f"🔄 单击出战（精灵{chosen_slot}，目标={target_pet_type}）", "INFO")
            battle_key = "战斗.出战"
            try:
                self._click_region(battle_key, use_foreground)
            except KeyError:
                self._emit(f"⚠️ 找不到出战区域，切换失败", "WARN")
                return False
            self._emit(f"✅ [智能切换] 已出战精灵{chosen_slot}", "SUCCESS")
            return True
    ...
    def _switch_pet_for_rare_mode(
        ...
        与尼奥线相同色探针规则：出战偏灰为闪光波克尔；偏蓝则技能四偏红为艾斯菲格、偏蓝白为亚梅丝。
        识别到目标即出战；否则试下一槽；均不匹配则回退精灵一。失败回退四字/五字探针。
        ...
        try:
            ...
            if target_pet_type == "jita":
                if self._jita_pos:
                    ...
                    self._emit(f"🔍 [智能切换-稀有] 根据背包缓存先检测精灵{first_pet_num}（机塔）", "INFO")
                else:
                    first_pet_num = 2
                    self._emit("⚠️ [智能切换-稀有] 无机塔槽缓存，默认先检测精灵2", "WARN")
                ...
                if self._aisifeige_pos:
                    first_pet_num = 2 if self._aisifeige_pos == "二" else 3
                    self._emit(
                        f"🔍 [智能切换-稀有] 根据背包扫描结果，先检测精灵{first_pet_num}（艾斯菲格/艾菲德斯）",
                        "INFO",
                    )
                else:
                    # 如果背包扫描失败，使用默认逻辑（精灵3）
                    first_pet_num = 3
                    self._emit(
                        f"⚠️ [智能切换-稀有] 背包扫描结果不可用，使用默认逻辑（精灵3）",
                        "WARN",
                    )
            ...
            last_scanned: Optional[int] = None
            for pet_num in slot_scan_order:
                self._emit(f"🔍 [智能切换-稀有] 扫描精灵{pet_num}（色探针优先）", "INFO")
                sk = f"对战.切换精灵.切换精灵{idx_cn[pet_num]}"
                try:
                    self._click_region_twice(sk, use_foreground)
                except KeyError:
                ...
                time.sleep(SWITCH_PRE_READ_OUT_PROBE_SEC)
                self._current_switching_pet_num = pet_num
                pt = self._resolve_pet_type_at_current_switch_slot(use_foreground, "rare")
                results[pet_num] = pt
                last_scanned = pet_num
                self._emit(f"📋 [智能切换-稀有] 精灵{pet_num} → {pt or '未知'}", "INFO")
                if pt == target_pet_type:
                    chosen_slot = pet_num
                    self._emit(
                        f"✅ [智能切换-稀有] 精灵{pet_num} 已匹配目标（{target_pet_type}），直接出战",
                        "SUCCESS",
                    )
                    break
                if (
                    target_pet_type == "yameisi"
                    and getattr(self, "_pickmode_yameisi_branch", False)
                    and pt == "jita"
                ):
                    chosen_slot = pet_num
                    self._emit(
                        "✅ [智能切换-稀有·Pick亚梅丝] 技能四仍为机塔色型，按亚梅丝槽出战",
                        "SUCCESS",
                    )
                    break
            ...
            if chosen_slot is None:
                if (
                    target_pet_type == "yameisi"
                    and getattr(self, "_pickmode_yameisi_branch", False)
                    and first_pet_num is not None
                ):
                    chosen_slot = first_pet_num
                    self._emit(
                        "⚠️ [智能切换-稀有·Pick亚梅丝] 色探针未匹配，按背包缓存槽出战",
                        "WARN",
                    )
                else:
                    self._emit(
                        "⚠️ [智能切换-稀有] 未匹配到目标类型，回退假定精灵1为目标",
                        "WARN",
                    )
                    chosen_slot = 1
            ...
            try:
                ...
                self._emit(f"⚠️ 找不到出战区域，切换失败", "WARN")
                return False
            self._emit(f"✅ [智能切换-稀有] 已出战精灵{chosen_slot}", "SUCCESS")
            return True
    ...
    def _scan_pet_probes_to_identify_pets(self, use_foreground: bool, mode: str = "nieo") -> Tuple[Optional[str], Optional[str]]:
        """
        **旧**尼奥/稀有战后扫描（与 Pick 的 _scan_pickmode_bag_slots_skill4 / pickmode 判色不是同一套）。
        依次双击精灵二、精灵三，读「精灵背包.技能四探针」均值：
        - 偏红 → 艾斯菲格
        - 偏蓝白 → 尼奥：闪光艾菲亚；稀有：亚梅丝
        精灵一固定为波克尔/闪光波克尔，不参与本扫描。
        
        Returns:
            (pet1_pos, pet2_pos)：尼奥为 (闪光艾菲亚位, 艾斯菲格位)；稀有为 (亚梅丝位, 艾斯菲格位)
        """
        _ = use_foreground
        if mode == "nieo":
            self._emit(
                "🔍 [背包探针] 扫描精灵二、三（精灵背包.技能四探针：红=艾斯菲格，蓝白=闪光艾菲亚）",
                "INFO",
            )
            pet1_name = "闪光艾菲亚"
        else:
            self._emit(
                "🔍 [背包探针] 扫描精灵二、三（精灵背包.技能四探针：红=艾斯菲格，蓝白=亚梅丝）",
                "INFO",
            )
            pet1_name = "亚梅丝"

        slot_kind: Dict[str, Optional[str]] = {}
        for pos_cn in ("二", "三"):
            self._click_bag_pet_slot_double(pos_cn, use_foreground)
            time.sleep(0.55)
            rgb = self._mean_rgb_for_region_key("精灵背包.技能四探针")
            if rgb is None:
                self._emit(f"⚠️ [背包探针] 精灵{pos_cn} 无法读取技能四探针", "WARN")
                slot_kind[pos_cn] = None
                continue
            r, g, b = rgb
            kind = self._classify_bag_skill4_mean_rgb(rgb, mode)
            slot_kind[pos_cn] = kind
            self._emit(
                f"📋 [背包探针] 精灵{pos_cn} RGB=({r},{g},{b}) → {kind}",
                "INFO",
            )

        pet1_pos: Optional[str] = None
        pet2_pos: Optional[str] = None
        for pos_cn in ("二", "三"):
            k = slot_kind.get(pos_cn)
            if k == "aisifeige":
                pet2_pos = pos_cn
            elif k in ("flash_aifeia", "yameisi"):
                pet1_pos = pos_cn

        if pet1_pos and not pet2_pos:
            pet2_pos = "三" if pet1_pos == "二" else "二"
            self._emit(
                f"📋 [背包探针] 仅辨明{pet1_name}，推断艾斯菲格=精灵{pet2_pos}",
                "INFO",
            )
        elif pet2_pos and not pet1_pos:
            pet1_pos = "三" if pet2_pos == "二" else "二"
            self._emit(
                f"📋 [背包探针] 仅辨明艾斯菲格，推断{pet1_name}=精灵{pet1_pos}",
                "INFO",
            )
        elif pet1_pos and pet2_pos:
            self._emit(
                f"✅ [背包探针] {pet1_name}=精灵{pet1_pos}，艾斯菲格=精灵{pet2_pos}",
                "SUCCESS",
            )
        else:
            self._emit("❌ [背包探针] 未能识别精灵二/三类型", "WARN")
    ...
    def _handle_hengmu_before_to_script(
        ...
        try:
            ...
            if scan_hp_bars:
                ...
                if not self._try_select_pet_four_with_timeout(use_foreground, stop_event):
                    self._emit(
                        f"⚠️ [亨姆检测] {self.SELECT_PET_FOUR_TIMEOUT_S:.0f}秒内选中四一直未选中，背包未稳定，执行重连",
                        "WARN",
                    )
    ...
    def _check_capture_verify_four_color(self, use_foreground: bool) -> Optional[int]:
        """
        Pick 默认队伍下不再做「捕捉验证四」深蓝比对，以免误触发刷新重连。
    ...
    def _check_selected_pet_color(
        ...
        try:
            ...
            mean_rgb = np.round(arr.mean(axis=(0, 1))).astype(int)
            r, g, b = int(mean_rgb[0]), int(mean_rgb[1]), int(mean_rgb[2])
            log_detail = self._emit if verbose else lambda *a, **k: None
            log_detail(f"📋 [选中{pet_position}检测] 平均RGB: ({r}, {g}, {b})", "DEBUG")
            ...
            log_detail(
                f"📋 [选中{pet_position}检测] 距离黄色混合参考: {dist_to_yellow_mixed:.2f}，距离蓝色参考: {dist_to_blue:.2f}",
                "DEBUG",
            )
            ...
            log_detail(
                f"📋 [选中{pet_position}检测] RGB黄色得分: {rgb_yellow_score:.2f}，RGB蓝色得分: {rgb_blue_score:.2f}",
                "DEBUG",
            )
            ...
            if dist_to_yellow_mixed < dist_to_blue:
                ...
                if rgb_yellow_score > 1.5:
                    is_selected = True
                    log_detail(
                        f"✅ [选中{pet_position}检测] 距离黄色混合更近且RGB特征偏向黄色，判定为已选中",
                        "DEBUG",
                    )
                elif rgb_blue_score > 0.8:
                    is_not_selected = True
                    log_detail(
                        f"✅ [选中{pet_position}检测] 虽然距离黄色混合更近，但RGB特征明显偏向蓝色，判定为未选中",
                        "DEBUG",
                    )
                else:
                    is_selected = True
                    log_detail(
                        f"✅ [选中{pet_position}检测] 距离黄色混合更近，判定为已选中",
                        "DEBUG",
                    )
            else:
                # 距离蓝色更近
                if rgb_blue_score > 0.6:
                    is_not_selected = True
                    log_detail(
                        f"✅ [选中{pet_position}检测] 距离蓝色更近且RGB特征偏向蓝色，判定为未选中",
                        "DEBUG",
                    )
                elif rgb_yellow_score > 2.0:
                    is_selected = True
                    log_detail(
                        f"✅ [选中{pet_position}检测] 虽然距离蓝色更近，但RGB特征明显偏向黄色，判定为已选中",
                        "DEBUG",
                    )
                else:
                    is_not_selected = True
                    log_detail(
                        f"✅ [选中{pet_position}检测] 距离蓝色更近，判定为未选中",
                        "DEBUG",
                    )

            # 返回结果
            if is_selected:
                self._emit(f"✅ [选中{pet_position}检测] 已选中（RGB=({r},{g},{b})）", "SUCCESS")
                return 1
            elif is_not_selected:
                if verbose:
                    self._emit(
                        f"📋 [选中{pet_position}检测] 未选中（RGB=({r},{g},{b})）",
                        "DEBUG",
                    )
                return 0
            else:
    ...
    def _try_select_pet_four_with_timeout(
        ...
        """
        战后放回仓库前置：快速双击精灵四，检测到选中四高亮则返回 True，超时返回 False（触发重连四）。
        """
        timeout_s = (
            float(timeout_s)
            if timeout_s is not None
            else self.SELECT_PET_FOUR_TIMEOUT_S
        )
        check_delay = self.SELECT_PET_FOUR_CHECK_DELAY_S
        click_interval = self.SELECT_PET_FOUR_CLICK_INTERVAL_S

        self._emit(
            f"🔍 [放回仓库] 前置：双击精灵四并检测选中四，{timeout_s:.0f}秒内未选中则重连四",
            "INFO",
        )
        pet_key = "精灵背包.精灵四"
        ...
        while (time.time() - start_time) < timeout_s:
            ...
            if result == 1:
                self._emit(
                    f"✅ [放回仓库] 检测到选中四（第{attempt}次，耗时{time.time() - start_time:.1f}s）",
                    "SUCCESS",
                )
                return True
            now = time.time()
            if result == 0 and (now - last_progress_log) >= 2.0:
                elapsed = now - start_time
                self._emit(
                    f"⏳ [放回仓库] 选中四仍为未选中，继续双击（第{attempt}次，已等待{elapsed:.1f}/{timeout_s:.0f}s）",
                    "INFO",
                )
                last_progress_log = now
        self._emit(
            f"⚠️ [放回仓库] {timeout_s:.0f}秒内选中四一直未选中（共{attempt}次双击），判定需重连四",
            "WARN",
        )
        return False
    ...
    def set_non_mantis_capsule_tier(self, tier: str) -> None:
        """
        投捕捉胶囊策略：
        - cycle / default：超→超→特 循环（默认）
        - super / high / special：全程单档备选
        """
    ...
    def _capsule_cycle_tiers_for_current_battle(self) -> Optional[Tuple[str, ...]]:
        """
        捕捉投掷循环档位。
        默认：超→超→特（`DEFAULT_CAPSULE_CYCLE_TIERS`）；
        Dashboard 选仅超级/仅高级/仅特级时为单档备选。
        螳螂对战敌方野生 122 时首回合无敌由 invincible_first_round 决定，与本循环无关。
        """
        t = getattr(self, "_non_mantis_capsule_tier", "cycle")
    ...
    def _capsule_cycle_log_fields(self) -> Tuple[str, str]:
        ...
        if t == DEFAULT_CAPSULE_CYCLE_TIERS:
            return ("default_super_super_special", ",".join(t))
        if t == ("special",):
    ...
    def _rotation_running_bucket(self) -> Optional[str]:
        """
        当前实际跑在「轮换」的哪一段：nieo / shuangta（双塔含嘟咕噜等）。
        与 _detect_rotation_mode 的 expected 对比，用于发现「该切了但时间戳未触发」的情况。
        """
        m = getattr(self, "_current_mode", None)
        if m == "nieo":
            return "nieo"
        if m in ("shuangta", "dugulu"):
            return "shuangta"
        return None
    ...
    def _check_rotation_switch_time(self) -> bool:
        ...
        # ✅ 时间表与当前运行模式是否一致（不能只看 _next_rotation_switch_time，否则午夜后仍跑尼奥时
        #    若 next 被同步成「当日 19:55」会一直等到晚上才切）
        ...
        if running_bucket is not None:
            ...
            if running_bucket != expected_mode:
                ...
                self._last_rotation_time_check = 0.0
                self._emit(
                    f"⏰ [轮换检测] 时间表与运行不一致：当前跑={running_bucket}，此时应为={expected_mode}，"
                    f"立即切换（已同步下次切换点 {canonical_next.strftime('%Y-%m-%d %H:%M:%S')}）",
                    "SYSTEM",
                )
                return True
    ...
    def _check_nieo_reconnect_condition(self, use_foreground: bool, stop_event: threading.Event, force_reconnect: bool = False) -> None:
        ...
        # ✅ 修改：只需要1次超过「最小值×PETSWF_TO_PETITEM_MIN_MULTIPLIER」阈值就触发重连，或者超过硬线
        should_reconnect = False
        reconnect_reason = ""
        _pm = self.PETSWF_TO_PETITEM_MIN_MULTIPLIER
        ...
        if not should_reconnect:
            if (self._petswf_to_petitem_current_duration is not None and 
                ...
                threshold = self._petswf_to_petitem_min_duration * _pm
                # ✅ 修复：添加调试日志，确保逻辑正确，并改进日志输出显示最小值和阈值
                self._emit(f"🔍 [重连检查-尼奥模式] 当前时间={self._petswf_to_petitem_current_duration:.3f}s, 最小值={self._petswf_to_petitem_min_duration:.3f}s, 阈值(×{_pm:g})={threshold:.3f}s, 连续超过次数={self._petswf_to_petitem_consecutive_over_threshold}", "DEBUG")
                ...
                if self._petswf_to_petitem_current_duration >= threshold:
                    ...
                    if self._petswf_to_petitem_consecutive_over_threshold >= 1:
                        should_reconnect = True
                        reconnect_reason = f"petswf到PetItem时间差超过最小值的{_pm:g}倍 (当前={self._petswf_to_petitem_current_duration:.3f}s, 最小值={self._petswf_to_petitem_min_duration:.3f}s, 阈值={threshold:.3f}s)"
                else:
                    # 当前时间未超过阈值，重置连续超过计数
    ...
    def _check_pure_energy_reconnect_condition(self, use_foreground: bool, stop_event: threading.Event, force_reconnect: bool = False) -> None:
        """与 `_check_nieo_reconnect_condition` 同类：时间/硬线重连，`to纯净能量.json` Step3."""
        pem_pre = getattr(self, "_pem_pre_rotation_first", False)

        if force_reconnect:
            battles_since_last_reconnect = self._battle_count - self._last_reconnect_battle_count
            self._emit(f"⚠️ [重连检查-纯净能量] 连续入战失败，强制执行重连", "WARN")
            self._emit(f"📊 [重连统计-纯净能量] 本次重连前战斗 {battles_since_last_reconnect} 次", "INFO")
            from core.dar_route_runner import DEFAULT_PROFILE_DUGULU
            temp_profile = DEFAULT_PROFILE_DUGULU
            self._current_mode = "pure_energy"
            self._execute_refresh_reconnect(
                temp_profile, use_foreground, stop_event, reason="纯净能量-强制重连", retry_count=0, max_retries=None
            )
            self._petswf_to_petitem_min_duration = None
            self._petswf_to_petitem_current_duration = None
            self._petswf_to_petitem_consecutive_over_threshold = 0
            ...
            if getattr(self, "_should_restart_after_reconnect", False):
                ...
                self.run_pure_energy_resource_mode(
                    stop_event=threading.Event(),
                    use_foreground=use_foreground,
                    nieo_pre_rotation_first=pem_pre,
                    skip_nie_77=getattr(self, "_skip_nie_77", False),
                )
            return
        ...
        if self._petswf_to_petitem_current_duration is not None:
            if self._petswf_to_petitem_current_duration >= self.PETSWF_TO_PETITEM_HARD_LIMIT_SEC:
                should_reconnect = True
                reconnect_reason = (
                    f"petswf到PetItem ({self._petswf_to_petitem_current_duration:.3f}s) "
                    f"超过{self.PETSWF_TO_PETITEM_HARD_LIMIT_SEC:.1f}s硬线"
                )
        if not should_reconnect:
            if self._petswf_to_petitem_current_duration is not None and self._petswf_to_petitem_min_duration is not None:
                threshold = self._petswf_to_petitem_min_duration * _pm
                self._emit(
                    f"🔍 [重连检查-纯净能量] 当前={self._petswf_to_petitem_current_duration:.3f}s, "
                    f"最小={self._petswf_to_petitem_min_duration:.3f}s, "
                    f"阈值(×{_pm:g})={threshold:.3f}s",
                    "DEBUG",
                )
                if self._petswf_to_petitem_current_duration >= threshold:
                    self._petswf_to_petitem_consecutive_over_threshold += 1
                    if self._petswf_to_petitem_consecutive_over_threshold >= 1:
                        should_reconnect = True
                        reconnect_reason = (
                            f"petswf到PetItem超过最小值{_pm:g}倍 "
                            f"(当前={self._petswf_to_petitem_current_duration:.3f}s)"
                        )
                else:
                    self._petswf_to_petitem_consecutive_over_threshold = 0
        ...
        if should_reconnect:
            battles_since_last_reconnect = self._battle_count - self._last_reconnect_battle_count
            self._emit(f"⚠️ [重连检查-纯净能量] {reconnect_reason}，执行重连", "WARN")
            self._emit(f"📊 [重连统计-纯净能量] 重连前战斗 {battles_since_last_reconnect} 次", "INFO")
            from core.dar_route_runner import DEFAULT_PROFILE_DUGULU
            temp_profile = DEFAULT_PROFILE_DUGULU
            self._current_mode = "pure_energy"
            self._execute_refresh_reconnect(
                ...
                use_foreground,
                stop_event,
                reason=f"纯净能量-{reconnect_reason}",
                retry_count=0,
                max_retries=None,
            )
            self._petswf_to_petitem_min_duration = None
    ...
    def _get_rotation_place_pets_mode(self, profile: Optional[WildCaptureProfile]) -> str:
        ...
        if "尼奥" in name_lower:
            return "nieo"
        # 历史名「双塔」：含义已变为「非尼奥 profile」分支标签，取宠与上相同
        return "shuangta"
    ...
    def _execute_unified_wild_rare_pre_reconnect(
        ...
        """
        野外稀有**统一前置重连**（适用于：**嘟咕噜 / 双塔 / 小豆芽 / 眼球**；螳螂/闪光皮皮仍走各自专用前置）。

        流程：**Step1+Step2**（含基地 **map500001** 门控）→ **分子转化仪（强制每次执行，忽略 30min 节流）** → 清空背包 → Pick 轮换派驻精灵 → 身边跟随 → 执行 ``to*``.json → **验地图零（newNPC）**；不通过则返回 False（调用方可整管 ``_execute_refresh_reconnect`` 或重试本函数）。

        成功后由 ``bot_thread`` 调用 ``run()``：其内仍含 **背包恢复 + 地图零→一入口**（与「先做前置再直接 run」约定一致）。

        **与已有专用前置对比（实现层摘要）**

        Step3 仓库 Pick **已全模式统一**（波克尔 / 艾菲德斯 / 机塔路径，见 ``_pickmode_rotation_step3_place_pets``）；下列差异仅在 **to 脚本、分子仪节流、地图门控**，不在取宠点击序列。

        - **闪光** ``_execute_flash_pipi_pre_rotation_reconnect``：与统一管线一致；当前引擎默认 ``_unified_reconnect_pipeline_always``，Step1+2 分子仪 **强制**。**仅** ``to闪光皮皮``+验 **11**，不点 **11→10**（进 10 在 ``run()``）。
        - **螳螂** ``_execute_mantis_pre_rotation_reconnect``：同上，分子仪 **强制**。``to螳螂`` 后 **立刻执行地图 ``10to11`` + 等 map11**；与本统一前置（只验地图零、不进 11）不同，**不可合并为同一条**。
        - **尼奥/纯净** ``_execute_nieo_pre_rotation_reconnect``：``to尼奥`` / ``to纯净能量``+验 **11/26**；与野外稀有/本条 **同一套 Pick**，旧文案「尼奥三 / 双塔四」仅标签遗留。
        - **本条统一前置**：**强制分子仪**；Pick 同上；字面地图零见 ``wild_rare_map_gate_zero_literal_swf_ids``（嘟咕噜 320、双塔 315、豆芽 11 与零同字面、闪光 11、眼球 61|62）；随后 ``run()`` 负责 **315→320** / **320→323** / …
        """
        if self._rotation_reconnect_executing:
            self._emit("⚠️ [野外统一前置] 已有轮换前置/专用前置在执行，忽略", "WARN")
            return False

        reason_tag = "野外稀有-统一前置重连"
        expected_zero = self._wild_rare_expected_map_zero_after_to(profile)
        if expected_zero is None:
            self._emit(f"⚠️ [{reason_tag}] profile={profile.name} 不支持统一前置（期望地图零未配置）", "WARN")
            return False

        to_script = self._get_to_script_name(profile)
        if not to_script:
            self._emit(f"⚠️ [{reason_tag}] 无法解析 to 脚本名：{profile.name}", "WARN")
            return False
        ...
        try:
            ...
            self._emit(
                f"🔄 [{reason_tag}] 开始：{profile.name} → 基地门控+必跑分子仪 → Pick → {to_script} → 验地图零={zero_banner}",
                "SYSTEM",
            )
            ...
            if not self._rotation_step1_login(
                ...
                self._emit(f"❌ [{reason_tag}] Step1+2（刷新/基地门控）失败", "ERROR")
                return False
            ...
            if not self._rotation_clear_backpack_and_pick_or_skip(
                place_mode, use_foreground, stop_event, log_tag=reason_tag
            ):
                self._emit(f"⚠️ [{reason_tag}] 清空背包未完成", "WARN")
                return False
            ...
            if not self._rotation_step4_set_companion(use_foreground, stop_event):
                self._emit(f"⚠️ [{reason_tag}] 设置身边跟随失败", "WARN")
                return False
            ...
            self._emit(f"📜 [{reason_tag}] 执行 {to_script}.json", "SYSTEM")
            if not hasattr(self.bot, "daily_runner"):
                self._emit(f"⚠️ [{reason_tag}] bot.daily_runner 不存在", "WARN")
                return False
            if not self._to_script_has_steps(to_script):
                self._emit(
                    f"❌ [{reason_tag}] {to_script}.json 无步骤，请用「脚本录制器」补录后再运行",
                    "ERROR",
                )
                return False
            if not self.bot.daily_runner.run_single_script(to_script, bg_mode=bg_mode):
                self._emit(f"⚠️ [{reason_tag}] {to_script}.json 执行失败", "WARN")
                return False
            ...
            if (
                ...
                self._emit(
                    f"✅ [{reason_tag}] 地图零通过：map={last_map_id}+newNPC，可启动 {profile.name} 的 run()（内含恢复+进图）",
                    "SUCCESS",
                )
            ...
            self._emit(
                f"❌ [{reason_tag}] 地图零校验失败（期望 map={expected_zero}，实际 map={last_map_id}, has_newNPC={has_newNPC}）",
                "ERROR",
            )
            return False
        finally:
    ...
    def _to_script_has_steps(self, to_script: str) -> bool:
        """检查 fix_script/{to_script}.json 是否存在且 steps 非空。"""
    ...
    def _start_normal_1and1_monitoring(
        ...
        4. 如果出现，等待0.5s后终止当前任务并执行标准刷新重连（三键→Login→程序化登录→再接轮换/to），不再直接跳进重连核心
        5. 如果没出现，继续监控
            ...
            while (
                ...
                if login_swf_check_active and login_swf_check_start_cursor is not None and login_swf_check_start_time is not None:
                    ...
                    if detected:
                        ...
                        # ✅ 断线 Login 时客户端仍在旧地图：必须先走三键刷新管线（_execute_refresh_reconnect），
                        #    不能再直接 _execute_reconnect_scripts_*（否则会误用「登录后门控」把旧图当登录态）。
    ...
    def _on_map_zero_gate_passed(self) -> None:
        """地图零门控通过：下次 run() 打开精灵背包恢复前先等待 MAP_ZERO_TO_RUN_RECOVER_DELAY_SEC。"""
    ...
    def _sleep_after_map_zero_before_run_recover(self, stop_event: threading.Event) -> None:
        ...
        self._emit(
            f"⏳ 地图零已通过，等待{delay:.1f}s后再打开精灵背包（run恢复）",
            "INFO",
        )
    ...
    def _check_last_map_and_newnpc(
        ...
        *,
        ...
        3. 检查这个map序号是否与目标相符：默认等于 ``map_id``；若传入 ``accepted_map_ids``，
           则在集合内即通过（用于眼球 **to 后地图零**：61/62，而 ``map_id`` 仍传 profile 的 60）。
        
        Args:
            map_id: 要检测 / 日志展示的 map ID（稀有门控常为地图零的数字；眼球与 ``accepted_map_ids`` 联用）
            timeout_s: 超时时间（秒，未使用，保留用于兼容）
            initial_pause_s: 扫描前等待日志落盘的时间
            accepted_map_ids: 若为 None，要求 ``found == map_id``；否则要求 ``found in accepted_map_ids``。
        
        Returns:
            (检测到的 map_id, 是否检测到newNPC)，通过时第二个 True 且第一个为实测 id。
        """
        try:
            from core.logger import fetch_kernel_since, kernel_cursor
            ...
            self._emit(
                f"🔍 [地图检测] 从已出现的日志中扫描：找到第一个NewNPC，然后在其上方找到第一个map序号（{gate_desc}）",
                "INFO",
            )
            ...
            if acc is not None:
                ...
                if match:
                    self._emit(
                        f"✅ [地图检测] 确认：NewNPC 上方 map={found_map_id} ∈ 门控集合 {sorted(acc)}",
                        "SUCCESS",
                    )
                    return (int(found_map_id), True)
                self._emit(
                    f"⚠️ [地图检测] NewNPC 上方 map={found_map_id} 不在门控集合 {sorted(acc)}",
                    "WARN",
                )
                return (int(found_map_id), True)
    ...
    def _mark_refresh_pipeline_first_map_kernel_cursor(self, cursor: int) -> None:
        """首次检测到进图 map 时记录 loop 迭代起点 cursor，避免漏掉 map 行本身。"""
    ...
    def _mark_refresh_pipeline_map500001_gate_done(self) -> None:
        """
        全局 **Step2** 已成功（基地 + map500001 门控）后调用：下一进 `_execute_reconnect_scripts_*` 时 `_consume_reconnect_core_map500001_skip_if_set` 可跳过一次全缓冲 500001 复验。
        野外 `_execute_refresh_reconnect` 与轮换 `_rotation_step1_login` 在 Step2 成功后都应调用，语义一致。
        """
        self._reconnect_core_skip_map500001_fullcheck_once = True

    def _consume_reconnect_core_map500001_skip_if_set(self) -> bool:
        """Step3 入口：若 Step2 已 `_mark_refresh_pipeline_map500001_gate_done`，清除并返回 True，跳过后续全缓冲 500001 检查。"""
    ...
    def _step1_trinity_clicks_and_wait_login_swf(
        ...
        if not self._refresh_trinity_clicks(use_foreground, log_prefix=log_prefix):
            return "trinity_fail"
        time.sleep(0.45)
        self._emit(f"⏳ [Step1·{log_prefix}] 等待 Login.swf 内核行（{max_wait_s:.0f}s）…", "INFO")
        from core.logger import fetch_kernel_since, kernel_cursor
        ...
        while (time.time() - t0) < max_wait_s:
            if stop_event is not None and stop_event.is_set():
                self._emit(f"⛔ [Step1·{log_prefix}] 等待 Login 时被停止（stop_event）", "WARN")
                return "stopped"
            if getattr(self.bot, "stop_current", False):
                self._emit(f"⛔ [Step1·{log_prefix}] 等待 Login 时被停止（stop_current）", "WARN")
                return "stopped"
            lines = fetch_kernel_since(start_cursor)
            if isinstance(lines, list):
                for line in lines:
                    if self._kernel_line_indicates_login_swf(line):
                        self._emit(f"✅ [Step1·{log_prefix}] 已检测到 Login.swf（本阶段结束）", "SUCCESS")
                        return "login"
            start_cursor = kernel_cursor()
            time.sleep(0.5)
    ...
    def _click_refresh_setting_attach_client(
        ...
        """
        「刷新.设置」贴在主窗口左上角壳区：横坐标为区域 bbox **水平中心**的主窗口 client 像素 X，
        纵坐标固定 **client y = 45**（不按 1200×700 视口缩放）。
        """
        lp = f"{log_prefix} " if log_prefix else ""
        try:
        ...
        if ok:
            self._emit(
                f"🖱️ [{lp}刷新UI] 刷新.设置→主窗口 client ({client_x},{client_y})，未走视口缩放",
                "INFO",
            )
        else:
            self._emit(f"❌ [{lp}刷新UI] 刷新.设置 click_client 失败", "ERROR")
        return ok
    ...
    def _click_three_keys_until_map_refresh(
        ...
        刷新登录进图循环（更新版）：
        - 先循环点击：普通确认 ↔ 刷新.登录
        - 直到「巅峰对战.登录探针」从蓝色变为（近似）纯白
        - 再对 服务器一/二/三 探针做 OCR（数字 1/2/3 随机分配），点击 OCR=3 对应服务器区域
        - 直到 kernel 出现 map 信号
        """
        from core.logger import fetch_kernel_since, kernel_cursor
        ...
        # Phase A：普通确认 ↔ 刷新.登录 循环，直到登录探针变白
        click_sequence = [
        ...
        while True:
            ...
            if time.time() - t0 > max_wait_s:
                self._emit(
                    f"⚠️ [{log_tag}] 刷新登录循环超时（{max_wait_s:.0f}s）",
                    "WARN",
                )
                return False
            ...
            if now - last_click >= interval:
                ...
                if not clicked and current_keys and current_keys[0].startswith("刷新."):
                    self._emit(f"❌ [{log_tag}] 缺少 region：{current_keys[0]}", "ERROR")
                    return False
                seq_idx = (seq_idx + 1) % len(click_sequence)
                last_click = now
            time.sleep(poll)

        # Phase B：登录探针已白 → OCR 三块服务器探针，点 OCR=3 的服务器直到出现 map
        self._emit(f"✅ [{log_tag}] 登录探针已变为纯白，开始 OCR 选服", "SUCCESS")

        probe_to_server_click = [
            ("刷新.服务器一探针", ("巅峰对战.服务器一", "刷新.服务器一")),
            ("刷新.服务器二探针", ("巅峰对战.服务器二", "刷新.服务器二")),
            ("刷新.服务器三探针", ("巅峰对战.服务器三", "刷新.服务器三")),
        ]

        chosen_click_key: Optional[str] = None
        if not pytesseract:
            self._emit(f"⚠️ [{log_tag}] pytesseract 不可用，默认点服务器三", "WARN")
            ...
            if probe_key_3:
                ...
                self._emit(f"⚠️ [{log_tag}] 未识别到数字3，默认点服务器三", "WARN")
                for pk, click_keys in probe_to_server_click:
                    if "服务器三探针" in pk:
                        for ck in click_keys:
                            if _region_exists(ck):
        ...
        if not chosen_click_key:
            self._emit(f"❌ [{log_tag}] 找不到可点击的服务器区域（服务器一/二/三）", "ERROR")
            return False

        self._emit(f"🖱️ [{log_tag}] 选服：循环点击 {chosen_click_key} 直到出现 map", "INFO")
        cursor = kernel_cursor()
        last_click = 0.0
        while True:
            if stop_event.is_set() or getattr(self.bot, "stop_current", False):
                return False
            if time.time() - t0 > max_wait_s:
                self._emit(f"⚠️ [{log_tag}] 选服后等待 map 超时（{max_wait_s:.0f}s）", "WARN")
                return False
            now = time.time()
            if now - last_click >= interval:
                try:
                    ...
                    self._emit(f"❌ [{log_tag}] 缺少 region：{chosen_click_key}", "ERROR")
                    return False
                last_click = now
            loop_cursor = cursor
            lines = fetch_kernel_since(cursor)
    ...
    def _gate_map500001_after_refresh_base(
        ...
        *,
        ...
        force_molecule_converter：允许执行分子转化仪时为 True 则忽略 30min 节流，每次跑分子转化仪（野外统一前置重连用）。
        skip_molecule_converter：为 True 时本趟**完全不调用**分子转化仪流程（不更新节流时间戳）。
        另：Dashboard 未勾选「执行分子转化仪」时由调用方反映在 ``_enable_molecule_converter``，
        门控内等价于跳过（不更新时间戳）。
        """
        from core.logger import fetch_kernel_since, kernel_cursor
        ...
        def _maybe_run_molecule_converter_flow() -> bool:
            """
            门控完成后、打开背包前：按时间节流执行“分子转化仪”流程。
            - 第一次执行或距离上次执行超过 30min30s 才会执行
            - force_molecule_converter 时忽略节流，必跑
            """
        ...
        while time.time() - t0 < 32.0:
            ...
            if last_map == self.MAP_ID_AFTER_LOGIN and has_npc:
                ...
                if skip_molecule_converter:
                    ...
                    self._emit(f"⚠️ [{log_tag}] 分子转化仪流程执行失败（继续后续）", "WARN")
                self._emit(
    ...
    def _run_molecule_converter_flow_after_gate(
        ...
        *,
        ...
        - sleep 1.0
        - 双击 刷新.基地右侧
        - sleep 0.3（基地右侧第二次后，再双击分子转化仪）
        - 双击 刷新.分子转化仪
        - 第一次 1AND1：持续点至 1AND1 探针消失（超时 3s，同「直到消失」语义）
        - sleep 1.5（第一次 1AND1 结束后再点下一次分子转化仪）
        - 再双击 分子转化仪
        - sleep 0.5
        - 从“门控后 newNPC”往上（倒序）扫描内核日志片段：若同时出现 MoleculePanel.swf 与 doodle/icon/<id>.swf
          则执行「精元确认 + 1AND1（3s）+ 点击关闭分子转化仪」并重复，最多 3 次；若仍未触发则兜底执行一次
        """
        tag = f"{log_tag}-分子转化仪"
        ...
        def _double_click_any(keys: Sequence[str], gap: float = 0.10) -> bool:
            # 双击用两次 click_region，保持与现有 click 体系一致
            for _ in range(2):
        ...
        def _confirm_1and1_until_disappear(timeout_s: float = 3.0) -> bool:
            """
            1AND1：持续点击确认，直到 1AND1 探针消失（或超时）。首次双击分子转化仪后与精元分支中「直到消失」一致。

            不能仅凭「当前帧未匹配白+蓝探针」就判定已消失：对话框未完全弹出或首帧色差会导致误判，
            需先至少检测到一次 1AND1（双探针），再在连续若干次读不到探针后才认为已关闭。
            """
            time.sleep(0.22)
            t0d = time.time()
            seen_probes = False
        ...
        def _click_close_molecule_panel() -> bool:
            self._emit(f"🖱️ [{tag}] 点击 刷新.关闭分子转化仪", "INFO")
            if not _click_once_any(("刷新.关闭分子转化仪",)):
                self._emit(f"❌ [{tag}] 缺少/点击失败：刷新.关闭分子转化仪", "ERROR")
                return False
            self._sleep_abortable(stop_event, 0.2, tick=0.05)
            return True

        def _click_jingyuan_confirm_and_1and1_once() -> bool:
            self._emit(f"🖱️ [{tag}] 点击 刷新.精元确认", "INFO")
            if not _click_once_any(("刷新.精元确认",)):
                self._emit(f"❌ [{tag}] 缺少/点击失败：刷新.精元确认", "ERROR")
                return False
            self._emit(f"🟨 [{tag}] 1AND1确认直到消失（{molecule_1and1_timeout_s}s 超时）", "INFO")
        ...
        def _kernel_segment_has_molecule_and_doodle_icon() -> bool:
            """
            从 gate_start_cursor 开始取内核片段，倒序扫到“门控后 newNPC”那行就停止；
            在这段倒序窗口内若同时出现 MoleculePanel.swf 与 doodle/icon/<id>.swf 则返回 True。
            """
            try:
                seg = fetch_kernel_since(gate_start_cursor)
            except Exception:
            ...
            # ✅ 只看“最新那次 newNPC 之后（更新的）”：
            # 倒序从最新往旧扫，遇到第一条 newNPC 才开始统计；newNPC 之前（更旧）不计入。
            seen_newnpc = False
            for line in reversed(seg):
                s = str(line)
                if not seen_newnpc:
        ...
        self._emit(f"⏳ [{tag}] sleep 1.0", "INFO")
        self._sleep_abortable(stop_event, 1.0, tick=0.1)

        self._emit(f"🖱️ [{tag}] 双击 刷新.基地右侧", "INFO")
        if not _double_click_any(("刷新.基地右侧",)):
            self._emit(f"❌ [{tag}] 缺少/点击失败：刷新.基地右侧", "ERROR")
            return False

        self._emit(f"⏳ [{tag}] sleep 0.3（基地右侧第二次后，双击分子转化仪前）", "INFO")
        self._sleep_abortable(stop_event, 0.3, tick=0.1)

        self._emit(f"🖱️ [{tag}] 双击 刷新.分子转化仪", "INFO")
        if not _double_click_any(("刷新.分子转化仪",)):
            self._emit(f"❌ [{tag}] 缺少/点击失败：刷新.分子转化仪", "ERROR")
            return False

        self._emit(f"🟨 [{tag}] 第一次 1AND1：直到探针消失（{molecule_1and1_timeout_s}s 超时）", "INFO")
        if not _confirm_1and1_until_disappear(timeout_s=molecule_1and1_timeout_s):
            return False

        self._emit(f"⏳ [{tag}] sleep 1.5（第一次 1AND1 后，再双击分子转化仪前）", "INFO")
        self._sleep_abortable(stop_event, 1.5, tick=0.1)

        self._emit(f"🖱️ [{tag}] 再次双击 刷新.分子转化仪", "INFO")
        if not _double_click_any(("刷新.分子转化仪",)):
            return False

        self._emit(f"⏳ [{tag}] sleep 0.5（第二次双击后）", "INFO")
        ...
        for i in range(3):
            ...
            if _kernel_segment_has_molecule_and_doodle_icon():
                self._emit(
                    f"✅ [{tag}] 检测到 MoleculePanel + doodle/icon（第 {i+1}/3 次），执行精元确认+1AND1",
                    "SUCCESS",
                )
        ...
        if did == 0:
            self._emit(
                f"⚠️ [{tag}] 未检测到 MoleculePanel+doodle/icon，兜底执行一次精元确认+1AND1",
                "WARN",
            )
            if not _click_jingyuan_confirm_and_1and1_once():
                return False
    ...
    def _run_full_refresh_login_pipeline(
        ...
        *,
        ...
        """Login.swf 之后：程序化点击 → 屏蔽策略 →（可选）基地 → map500001 门控（可强制每次跑分子转化仪）。"""
    ...
    def run_refresh_login_test_cycle(
        ...
        *,
        ...
        """
        测试用：执行一次「刷新登录直到出现 map」的完整 Step1+Step2，
        成功后立刻再执行一次「三键刷新」（不保存）以便观察新逻辑是否稳定。

        - 仅用于测试 UI 按钮触发；不进入 Step3（不跑轮换/to 脚本）。
        """
        retries = 0
        while retries <= max_retries:
            ...
            if step1 != "login":
                retries += 1
                self._emit(
                    f"⚠️ [刷新登录测试] Step1 未进入 Login.swf（res={step1}），重试 {retries}/{max_retries}",
                    "WARN",
                )
                continue
            ...
            if not ok2:
                retries += 1
                self._emit(
                    f"⚠️ [刷新登录测试] Step2 失败，重试 {retries}/{max_retries}",
                    "WARN",
                )
                continue

            self._emit("✅ [刷新登录测试] 已检测到 map，立即再次执行三键刷新…", "SUCCESS")
            ok3 = self._refresh_trinity_clicks(
                use_foreground=use_foreground, log_prefix="刷新登录测试-再次刷新"
            )
            return bool(ok3)
        return False
    ...
    def run_refresh_login_until_map(
        ...
        *,
        ...
        """
        UI按钮用：执行「刷新登录直到检测到 map」。
        不做二次刷新；只要 Step1+Step2 成功即可返回 True。
        """
        retries = 0
        while retries <= max_retries:
            if stop_event.is_set() or getattr(self.bot, "stop_current", False):
            ...
            if step1 != "login":
                retries += 1
                self._emit(
                    f"⚠️ [刷新登录] Step1 未进入 Login.swf（res={step1}），重试 {retries}/{max_retries}",
                    "WARN",
                )
                continue
            ok2 = self._run_full_refresh_login_pipeline(
            ...
            retries += 1
            self._emit(
                f"⚠️ [刷新登录] Step2 失败，重试 {retries}/{max_retries}",
                "WARN",
            )
        return False
    ...
    def _execute_refresh_flow_and_wait_login(self, profile: WildCaptureProfile, use_foreground: bool, stop_event: threading.Event, retry_count: int = 0, max_retries: Optional[int] = None) -> None:
        """
        重连脚本内部失败时：从 **Step1 起整管** 重走 `_execute_refresh_reconnect`（Step1→Step2→Step3），再分发到各模式。
        """
        # ✅ 调用统一的刷新重连函数，标记为重试调用
        self._execute_refresh_reconnect(
            profile=profile,
    ...
    def _handle_stage3_round_timeout_reconnect(
        ...
        try:
            ...
            if pre_path:
                self._emit(f"📸 [{reason}] 重连前已保存 client 截图：{pre_path}", "INFO")
        except Exception as e:
            self._emit(f"⚠️ [{reason}] 重连前截图失败: {e}，继续重连", "WARN")
        ...
        # ✅ 重要：即使战斗在回合内卡死并触发重连，也要把本场写入 battle_log（用于分析卡死/掉线）
        try:
            ubf = self._unified_framework
            cycle_mode, cycle_tiers = self._capsule_cycle_log_fields()
            enemy_ids = self._enemy_pet_ids_for_battle_csv(profile)
            ...
            self._emit(f"🧾 [{reason}] 已记录卡死退出战斗日志（result=round_timeout）", "INFO")
        except Exception:
            pass
    ...
    def _execute_refresh_reconnect(
        ...
        入口后、Step1 前：**尝试删除游戏目录下 ``tmp``**（``_try_delete_game_tmp_dir``），与 ``_current_mode`` 无关，凡走本函数均会尝试清缓存一次。
        ...
        2. **Step2**：`_run_full_refresh_login_pipeline`（至基地/map500001 门控；成功则 `_mark_refresh_pipeline_map500001_gate_done`）。
           若 ``_unified_reconnect_pipeline_always``（引擎默认开启）为 True，Step2 内强制跑分子转化仪（忽略 30min 节流）。
        ...
        try:
            ...
            # 1AND1 监控线程在断线 Login 后触发：主循环可能短暂置过 stop，不能挡掉「必须先三键刷新」的恢复路径
            is_1and1_login_refresh = "常态1AND1" in reason

            if stop_event.is_set() or getattr(self.bot, "stop_current", False):
                if is_map_entry_failure or is_1and1_login_refresh:
                    # 地图进入失败 / 1AND1 Login：清除停止标志以允许执行刷新重连
                    tag = "地图进入失败" if is_map_entry_failure else "1AND1 Login"
                    self._emit(f"🔄 [{reason}] 检测到停止标志，但这是{tag}触发的刷新重连，清除停止标志并继续执行", "INFO")
                    stop_event.clear()
                    if self.bot:
                        self.bot.stop_current = False
                else:
            ...
            # ✅ 记录“本 run 因重连结束”的原因（写入 battle_log 最后；同 run 只记一次）
            try:
            ...
            # ✅ 每次标准刷新重连均尝试删除游戏目录下 tmp（清客户端缓存）；失败不阻断重连
            try:
                self._emit(f"🗑️ [{reason}] 刷新重连：尝试删除游戏 tmp…", "INFO")
            ...
            if step1_res == "no_login":
                ...
                return
            # Step1 已完成；Step2：四下开始 → 屏蔽 → 基地 → map500001 门控
            self._emit(
                f"🔄 [Step2·{reason}] 开始程序化登录（至基地/map{self.MAP_ID_AFTER_LOGIN} 门控）",
                "INFO",
            )
            if is_retry:
            ...
            if not self._run_full_refresh_login_pipeline(
                ...
                self._emit(f"⚠️ [{reason}] Step2 程序化登录/基地门控失败，重新刷新", "WARN")
                self._execute_refresh_reconnect(
                    profile,
                    use_foreground,
                    stop_event,
            ...
            self._emit(
                f"🔄 [Step3·{reason}] Step2 已完成，开始重连 to/背包 阶段（若模式需要）",
                "INFO",
            )
    ...
    def _execute_refresh_flow_and_wait_login_for_nieo(self, use_foreground: bool, stop_event: threading.Event, retry_count: int = 0, max_retries: Optional[int] = None) -> None:
        """
        尼奥重连脚本内部失败时：从 **Step1 起整管** 重走 `_execute_refresh_reconnect`（Step1→Step2→Step3），再进入 `_execute_reconnect_scripts_for_nieo`。
        """
    ...
    def _execute_refresh_flow_and_wait_login_for_pure_energy(
        self, use_foreground: bool, stop_event: threading.Event, retry_count: int = 0, max_retries: Optional[int] = None
    ) -> None:
        """纯净能量 Step3 失败时：从 Step1 重走刷新，再回到 `to纯净能量`。须先 `_current_mode = pure_energy`。"""
        self._current_mode = "pure_energy"
        virtual_profile = WildCaptureProfile(
            name="纯净能量(资源)",
            route_hint="纯净能量",
    ...
    def _execute_reconnect_scripts_for_nieo(self, use_foreground: bool, stop_event: Optional[threading.Event] = None, retry_count: int = 0, max_retries: Optional[int] = None, is_capture_verify_four: bool = False) -> None:
        """
        **Step3（尼奥）**：须由 `_execute_refresh_reconnect` 已跑完 Step1+Step2 后调用。
        内容：map500001 轻量门控（可跳过）→ 轮换背包/精灵/跟随 → `to尼奥.json` → map11+newNPC 校验；失败则 `_execute_refresh_flow_and_wait_login_for_nieo` 从 Step1 重跑。
        """
        ...
        try:
            self._emit(
                "🔄 [Step3·重连脚本-尼奥] 开始（Step1/2 已完成）→ map 门控 / 轮换 / to尼奥 / 地图检测",
                "SYSTEM",
            )
            ...
            # Step3：map500001 门控（全缓冲；若 Step2 已门控通过则跳过一次）
            if not self._consume_reconnect_core_map500001_skip_if_set():
                # 登录后检查向上的第一个map是否为500001（与检测315逻辑相同），否则从 Step1 整管重跑
                ...
                self._emit("ℹ️ [重连脚本-尼奥] 已跳过全缓冲 map500001 复验（Step2 门控刚通过）", "INFO")
            
            # Step3·轮换子阶段：清空背包 → 取精灵 → 设置跟随
            temp_stop_event = threading.Event()
            self._emit("🔄 [Step3·重连脚本-尼奥] 轮换：清空背包、取精灵、设置跟随", "INFO")
            if not self._rotation_clear_backpack_and_pick_or_skip(
                "nieo", use_foreground, temp_stop_event, log_tag="重连脚本-尼奥"
            ):
    ...
    def _execute_reconnect_scripts_for_pure_energy(
        ...
        """
        **Step3（纯净能量）**：与尼奥同构：`to纯净能量.json` → 校验 **map 26 + newNPC**。
        """
        if stop_event and stop_event.is_set():
            self._emit("⛔ [重连脚本-纯净能量] 停止重试", "WARN")
            return
        if getattr(self.bot, "stop_current", False):
            self._emit("⛔ [重连脚本-纯净能量] stop_current被设置，停止重试", "WARN")
            return
        self._reconnect_scripts_executing = True
        try:
            self._emit(
                "🔄 [Step3·重连脚本-纯净能量] 开始 → map500001 / 轮换 / to纯净能量 / map26检测",
                "SYSTEM",
            )
            if not hasattr(self.bot, "daily_runner"):
                self._emit("⚠️ [重连脚本-纯净能量] bot.daily_runner 不存在", "WARN")
                return
            daily_runner = self.bot.daily_runner
            bg_mode = not use_foreground
            if not self._consume_reconnect_core_map500001_skip_if_set():
                ...
                if login_map_id != self.MAP_ID_AFTER_LOGIN or not login_has_newNPC:
                    actual_stop_event = stop_event if stop_event is not None else threading.Event()
                    self._execute_refresh_flow_and_wait_login_for_pure_energy(
                        use_foreground, actual_stop_event, retry_count, max_retries
                    )
                    return
            temp_stop_event = threading.Event()
            self._emit("🔄 [Step3·纯净能量] 轮换：清空背包、取三宠（波克尔/艾菲德斯/机塔，与野外稀有同 Pick）、设置跟随", "INFO")
            if not self._rotation_clear_backpack_and_pick_or_skip(
                "nieo", use_foreground, temp_stop_event, log_tag="重连脚本-纯净能量"
            ):
                actual_stop_event = stop_event if stop_event is not None else threading.Event()
                self._execute_refresh_flow_and_wait_login_for_pure_energy(
                    use_foreground, actual_stop_event, retry_count, max_retries
            ...
            if last_map_id == 26 and has_newNPC:
                ...
                if self._unified_framework and hasattr(self._unified_framework, "_petswf_to_petitem_durations"):
                    self._unified_framework._petswf_to_petitem_durations.clear()
                self._should_restart_after_reconnect = True
                self._emit("🔄 [重连脚本-纯净能量] 已置重启标志", "INFO")
                return
            max_retries_str = f"/{max_retries}" if max_retries is not None else ""
            self._emit(
                f"⚠️ [重连脚本-纯净能量] map 门控失败（map={last_map_id}, npc={has_newNPC}），"
                f"第 {retry_count + 1}{max_retries_str} 次刷新重试",
                "WARN",
            )
            actual_stop_event = stop_event if stop_event is not None else threading.Event()
            self._execute_refresh_flow_and_wait_login_for_pure_energy(use_foreground, actual_stop_event, retry_count, max_retries)
        except Exception as e:
            self._emit(f"❌ [重连脚本-纯净能量] 异常: {e}", "ERROR")
            import traceback
            self._emit(f"📋 {traceback.format_exc()}", "ERROR")
        finally:
            self._reconnect_scripts_executing = False
    ...
    def _execute_reconnect_scripts_for_shuangta(self, profile: WildCaptureProfile, use_foreground: bool, stop_event: Optional[threading.Event] = None, retry_count: int = 0, max_retries: Optional[int] = None, is_capture_verify_four: bool = False) -> None:
        """
        **Step3（双塔）**：`_execute_reconnect_scripts_core` 入口；map315+newNPC 为目标。须先经 `_execute_refresh_reconnect` 的 Step1+Step2。
        """
        if stop_event and stop_event.is_set():
    ...
    def _execute_reconnect_scripts_for_dugulu(self, profile: WildCaptureProfile, use_foreground: bool, stop_event: Optional[threading.Event] = None, retry_count: int = 0, max_retries: Optional[int] = None, is_capture_verify_four: bool = False) -> None:
        """
        **Step3（嘟咕噜）**：`_execute_reconnect_scripts_core` 入口；**地图零** map **320** + newNPC（与 ``to嘟咕噜`` 后内核一致；323 为 ``320to323`` 之后）。须先经 `_execute_refresh_reconnect` 的 Step1+Step2。
        """
        if stop_event and stop_event.is_set():
            self._emit("⛔ [重连脚本] 停止重试", "WARN")
            return
    ...
    def _execute_reconnect_scripts_for_rare_pet(
        ...
        """
        **Step3（稀有精灵）**：须由 `_execute_refresh_reconnect` 已跑完 Step1+Step2 后调用。
        内容：map500001 门控（可跳过）→ 轮换 → `to{profile}` → 目标 map 校验；失败则回到整管 `_execute_refresh_reconnect`。
        """
        ...
        try:
            self._emit(
                f"🔄 [Step3·重连脚本-{mode_name}] 开始（Step1/2 已完成）→ 门控 / 轮换 / {to_script_name}.json / 地图检测",
                "SYSTEM",
            )
            ...
            if not self._consume_reconnect_core_map500001_skip_if_set():
                ...
                self._emit(
                    f"ℹ️ [重连脚本-{mode_name}] 已跳过全缓冲 map500001 复验（Step2 门控刚通过）",
                    "INFO",
                )

            temp_stop_event = threading.Event()
            self._emit(f"🔄 [Step3·重连脚本-{mode_name}] 轮换：清空背包、取精灵、设置跟随", "INFO")
            if not self._rotation_clear_backpack_and_pick_or_skip(
                "shuangta", use_foreground, temp_stop_event, log_tag=f"重连脚本-{mode_name}"
            ):
            ...
            if (
                ...
                self._emit(f"✅ [重连脚本-{mode_name}] 地图门控通过：map={last_map_id}+newNPC，执行{mode_name}模式流程", "SUCCESS")
                self._on_map_zero_gate_passed()
                self._reconnect_scripts_executing = False
                self._petswf_to_petitem_min_duration = None
                self._petswf_to_petitem_current_duration = None
                ...
                if last_map_id is not None and not self._kernel_map_matches_profile_expectation(
                    ...
                    self._emit(f"⚠️ [重连脚本-{mode_name}] 检测到 map={last_map_id}（字面地图零应为 {literal_desc}），刷新重试（第 {retry_count + 1}{max_retries_str} 次）", "WARN")
                else:
                    self._emit(f"⚠️ [重连脚本-{mode_name}] 地图零门控未完成（字面零={literal_desc} map={last_map_id}, NPC={has_newNPC}），刷新重试（第 {retry_count + 1}{max_retries_str} 次）", "WARN")
                # ✅ 修复：重试时不能清除_executing标志
                actual_stop_event = stop_event if stop_event is not None else threading.Event()
                self._current_mode = mode_key
                self._execute_refresh_reconnect(
    ...
    def _execute_reconnect_scripts_core(self, profile: WildCaptureProfile, use_foreground: bool, stop_event: Optional[threading.Event] = None, retry_count: int = 0, max_retries: Optional[int] = None, is_capture_verify_four: bool = False) -> None:
        """
        **Step3（双塔/嘟咕噜/小豆芽等经 `to`+地图校验的 profile）**：
        须由 `_execute_refresh_reconnect` 已跑完 Step1+Step2。本函数：门控 / 轮换 / `to` 脚本 / 各模式目标 map+newNPC；失败则 `_execute_refresh_flow_and_wait_login` 从 Step1 重跑。
        """
        # 设置标志表示正在执行重连脚本
        self._reconnect_scripts_executing = True
        ...
        try:
            ...
            self._emit(
                f"🔄 [Step3·重连脚本·{profile.name}] 开始（Step1/2 已完成）→ map{self.MAP_ID_AFTER_LOGIN} 门控 / 轮换 / {to_script_name}.json / 地图检测",
                "SYSTEM",
            )
            ...
            # Step3：map500001 门控（全缓冲；未走 Step1+2 时失败会整管重跑）
            if not self._consume_reconnect_core_map500001_skip_if_set():
                # 登录后检查向上的第一个map是否为500001（与检测315逻辑相同），否则从 Step1 整管重跑
                login_map_id, login_has_newNPC = self._check_last_map_and_newnpc(self.MAP_ID_AFTER_LOGIN, timeout_s=10.0)
                if login_map_id != self.MAP_ID_AFTER_LOGIN or not login_has_newNPC:
                ...
                self._emit("ℹ️ [重连脚本] 已跳过全缓冲 map500001 复验（Step2 门控刚通过）", "INFO")
            
            # Step3·轮换子阶段
            rotation_mode = self._get_rotation_place_pets_mode(profile)
            temp_stop_event = threading.Event()
            self._emit(f"🔄 [Step3·重连脚本] 轮换：清空背包、取精灵、设置跟随（模式={rotation_mode}）", "INFO")
            if not self._rotation_clear_backpack_and_pick_or_skip(
                rotation_mode, use_foreground, temp_stop_event, log_tag="重连脚本"
            ):
            ...
            if "双塔" in profile_name_lower:
                ...
                if has_newNPC and self._kernel_map_matches_profile_expectation(
                    ...
                    self._emit(
                        "✅ [重连脚本] 双塔地图零通过：map=%s + newNPC，执行双塔流程"
                        % (last_map_id,),
                        "SUCCESS",
                    )
                    self._on_map_zero_gate_passed()
                    ...
                    self._emit(
                        "⚠️ [重连脚本] 地图零未通过（字面零=%s anchor=%s 检测到 map=%s, has_newNPC=%s），执行刷新流程并重试"
                ...
                if has_newNPC and self._kernel_map_matches_profile_expectation(
                    ...
                    self._emit(
                        "✅ [重连脚本] 嘟咕噜地图零通过：map=%s + newNPC，执行嘟咕噜流程"
                        % (last_map_id,),
                        "SUCCESS",
                    )
                    self._on_map_zero_gate_passed()
                    ...
                    gdesc = "|".join(str(x) for x in sorted(gz)) if gz else str(anchor)
                    self._emit(
                        f"⚠️ [重连脚本] 嘟咕噜地图零未通过（字面零={gdesc} anchor={anchor} "
                        f"map={last_map_id}, npc={has_newNPC}），第 {retry_count + 1}{max_retries_str} 次刷新重试",
                        "WARN",
                    )
    ...
    def _execute_reconnect_scripts(self, profile: WildCaptureProfile, use_foreground: bool, stop_event: Optional[threading.Event] = None, is_capture_verify_four: bool = False) -> None:
        """
        **Step3（通用）**：经 `_execute_refresh_reconnect` 的 Step1+Step2 后进入 `_execute_reconnect_scripts_core`。
        """
    ...
    def _sort_route_points_by_distance(
        ...
        *,
        ...
        """
        根据距离对路线点排序，并剔除若干「最远」点。

        「跳过」配额由调用方掌控：通常为手动剔除 + farthest_exclude_count，合计至多 2
        （例如 pem 不写 九点时 farthest_exclude_count=1）。
        ...
        Args:
            ...
            farthest_exclude_count: 剔除距参考点最远的前若干个点（默认 2）
        
        Returns:
            排序后的路线点列表（近的在前）；长度 = len(route_points) - farthest_exclude_count（不足则全长）
        """
        ...
        if map_id == 10:  # 尼奥一（10号地图）：使用"尼奥一.to二"区域
            ...
            reference_center = self._region_center(reference_reg)
        elif map_id == 11:  # map11（尼奥二）：使用"尼奥二.to一"区域
            ...
            if not reference_reg:
                ...
                return trim_farthest(route_points, k_drop)
            reference_center = self._region_center(reference_reg)
        elif map_id == 26:  # map26，路线前缀「纯净能量二」：取向 27 的入口
            reference_key = "纯净能量二.to一"
            reference_reg = self.regions.get(reference_key)
            if not reference_reg:
                self._emit(f"⚠️ 未找到参考点{reference_key}，使用默认顺序", "WARN")
                return trim_farthest(route_points, k_drop)
            reference_center = self._region_center(reference_reg)
        elif map_id == 27:  # map27，前缀「纯净能量一」：取向 26（``纯净能量一.to二``）
            reference_key = "纯净能量一.to二"
            reference_reg = self.regions.get(reference_key)
            if not reference_reg:
                self._emit(f"⚠️ 未找到参考点{reference_key}，使用默认顺序", "WARN")
            ...
            self._emit(f"⚠️ map_id={map_id} 未知的双图路线排序前缀，使用默认顺序", "WARN")
            return trim_farthest(route_points, k_drop)
    ...
    def _neighbor_switch_twomap_after_fail(
        ...
        *,
        ...
        """尼奥 10↔11、纯净能量 26↔27，或自定义三图 B↔C；成功切换后返回新 map id。"""
        ...
        if pem:
            # pem：与尼奥 10/11 同构的选 key 方式，仅录点文件名与坐标不同
            if current_map_id == 26:
                reg = self.regions.get("纯净能量二.to一")
                if not reg:
                    self._emit("⚠️ [纯净能量] 缺少纯净能量二.to一 区域", "WARN")
                    return None
                self._emit("🗺️ [纯净能量] 点击纯净能量二.to一 进入地图27", "SYSTEM")
                self._click_region(reg, use_foreground)
                if self._wait_for_map_id(27, stop_event, timeout_s=45.0):
                    return 27
            elif current_map_id == 27:
                reg = self.regions.get("纯净能量一.to二")
                if not reg:
                    self._emit("⚠️ [纯净能量] 缺少纯净能量一.to二 区域", "WARN")
                    return None
                self._emit("🗺️ [纯净能量] 点击纯净能量一.to二 返回地图26", "SYSTEM")
                self._click_region(reg, use_foreground)
                if self._wait_for_map_id(26, stop_event, timeout_s=45.0):
                    return 26
            return None
    ...
    def _pem_recover_pet_one_maintenance(self, use_foreground: bool, stop_event: threading.Event) -> None:
        """每若干场战斗后的精灵一体力恢复（与 _recover_pets 内『每40场』段落同构）。"""
        bag_open_key = "精灵背包.打开精灵背包"
        bag_open_btn_key = "精灵背包.打开精灵背包按钮"
        try:
            self._emit("💼 [纯净能量(资源)] 打开精灵背包（定期恢复精灵一）", "INFO")
            try:
                self._click_region(bag_open_btn_key, use_foreground)
            except KeyError:
                self._click_region(bag_open_key, use_foreground)
            ...
            if self._unified_framework:
                ...
                self._unified_framework._wait_for_confirm_probes(temp_config, timeout_s=2.0)
            self._sleep_abortable(stop_event, 0.5)
            self._emit("💼 恢复精灵一完成，关闭精灵背包", "INFO")
            try:
                self._click_region(bag_open_btn_key, use_foreground)
            except KeyError:
                self._click_region(bag_open_key, use_foreground)
            ...
            self._emit(f"⚠️ [纯净能量(资源)] 恢复精灵一异常: {e}，继续", "WARN")
    ...
    def _pem_pure_energy_calibration_reconnect_restart(
        ...
        self._emit(f"🔄 [纯净能量] 校准检测到非目标 map（{reason_tag}），刷新重连", "WARN")
        self._execute_refresh_reconnect(
            DEFAULT_PROFILE_DUGULU,
            use_foreground,
            stop_event,
            reason=f"校准后重连-纯净能量-{reason_tag}",
        )
        prof = getattr(self, "_configured_nieo_profile", None)
        if prof is not None:
    ...
    def _pem_pure_energy_maybe_rotation_then_exit(self, stop_event: threading.Event, tag: str) -> bool:
        if not self._check_rotation_switch_time():
            return False
        self._emit(
            f"✅ [轮换检测-纯净能量·{tag}] 到达切换时间，设置stop_event退出当前模式",
            "SUCCESS",
        )
        stop_event.set()
        return True

    def _pem_pure_energy_after_battle_counters(self, use_foreground: bool, stop_event: threading.Event) -> None:
        n = self.PEM_SKILL4_MAINTENANCE_EVERY_BATTLES
        if self._battle_count % n == 0:
            self._emit(
                f"🔄 [纯净能量] 已满{n}场，下场战斗前恢复精灵一（与战胜分支相同节奏）",
                "INFO",
            )
            self._pem_recover_pet_one_maintenance(use_foreground, stop_event)
        self._check_pure_energy_reconnect_condition(use_foreground, stop_event)
    ...
    def _pem_pure_energy_escape77_like_nieo(
        ...
        *,
        ...
        """勾选不捉 77 时：纯能量资源模式首回合逃跑，地图切换走 26↔27。"""
        from core.unified_battle_framework import BattleConfig, BattleMode
        from core.logger import kernel_cursor
        from core.dar_route_runner import DEFAULT_PROFILE_DUGULU

        self._emit("🎯 [纯净能量] 勾选不捕捉77尼尔，执行首回合逃跑（同尼奥逃跑）", "SYSTEM")
        self._is_in_battle = True
        self._is_scanning_steady_state = False
        ...
        self._battle_count += 1
        self._csv_log_battle("escape", [])
        self._emit("✅ [纯净能量] 77 尼尔逃跑完成", "SUCCESS")
        self._last_nie_family_id = None
        ...
        if getattr(self, "_unexpected_pet_reconnect_pending", False):
            self._unexpected_pet_reconnect_pending = False
            self._emit(
                "⚠️ [纯净能量-异常] 检测到非预期 pet ID，当前战斗后执行重连",
                "WARN",
            )
            self._check_pure_energy_reconnect_condition(use_foreground, stop_event, force_reconnect=True)
            return None
        self._pem_pure_energy_after_battle_counters(use_foreground, stop_event)
        return self._pem_pure_energy_neighbor_switch_finish(
            current_map_id=current_map_id,
            current_prefix=current_prefix,
            use_foreground=use_foreground,
    ...
    def _pem_pure_energy_nie_family_like_nieo(
        ...
        *,
        ...
        """尼尔家族（77/310/416）：技能一 → 切换 → 高级胶囊链路，战后恢复放回仓库对齐尼奥。"""
        from core.unified_battle_framework import BattleConfig, BattleMode, LastActionType
        from core.logger import kernel_cursor
        from core.dar_route_runner import DEFAULT_PROFILE_DUGULU

        self._emit(
            f"🎯 [纯净能量] 尼尔家族 nie_id={nie_id}：技能一切换捕捉（同源尼奥尼尔分支），战后恢复入仓",
            "SYSTEM",
        )
        self._is_in_battle = True
        self._is_scanning_steady_state = False
        ...
        if self._unified_framework._last_action == LastActionType.CAPSULE:
            self._csv_log_battle("captured", [nie_id] if nie_id else [])
            self._emit(f"✅ [纯净能量] 尼尔家族 {nie_id} 捕捉动作完成，执行恢复并入仓（同尼奥）", "SUCCESS")
            self._sleep_abortable(stop_event, 3.0)
            self._stop_1and1_monitoring = True
            self._is_recovering = True
            self._recover_pets(use_foreground, stop_event, skip_return_storage=False, nie_family_id=nie_id, profile=DEFAULT_PROFILE_DUGULU)
            self._is_recovering = False
            self._stop_1and1_monitoring = False
            self._last_nie_family_id = None
        else:
            self._emit("🎯 [纯净能量] 尼尔家族战胜（非胶囊收尾），检查后处理", "INFO")
            self._last_nie_family_id = None
    ...
    def _pem_after_change_skill4_defeat(
        ...
        *,
        ...
        """入战成功后：普通敌方全程技能四；遇尼尔家族走尼奥同源捕捉链；每场后可能 26↔27。返回下一 map id；None=应停止。"""
        from core.unified_battle_framework import BattleConfig, BattleMode
        from core.logger import kernel_cursor
        from core.dar_route_runner import DEFAULT_PROFILE_DUGULU
        ...
        if collected_pet_ids:
            ...
            self._emit(f"📋 [纯净能量] 已收集战斗精灵 ID: {pet_ids}", "SUCCESS")
        else:
            ids_set = self._collect_fight_pet_ids(timeout=4.5, collect_window=0.55, stop_event=stop_event)
            if not ids_set:
                self._emit("⚠️ [纯净能量] 未收集到战斗精灵 ID，切换地图重试", "WARN")
        ...
        if not (unique_ids & NIEO_EXPECTED_IDS):
            self._emit(
                "⚠️ [纯净能量提醒] 入战独特ID="
                + str(sorted(unique_ids))
                + "，不包含尼奥侧预期任一ID（12/312/418/1337/1459）；若误报可忽略",
                "WARN",
            )
        ...
        self._emit("🔍 [纯净能量] Stage2 等待 PetItem…", "INFO")
        stage2_cursor = kernel_cursor()
        ok, calib_result = self._unified_framework.stage2_calibration_and_petitem(
            trigger_callback=None,
            use_foreground=use_foreground,
        ...
        if calib_result == "reconnect_needed":
            self._emit("🔄 [纯净能量] 校准检测到非目标 map，刷新重连", "WARN")
            self._execute_refresh_reconnect(
                DEFAULT_PROFILE_DUGULU, use_foreground, stop_event, reason="校准后重连-纯净能量"
            )
            prof = getattr(self, "_configured_nieo_profile", None)
            if prof is not None:
                self.run_configured_nieo_mode(
        ...
        if not ok:
            self._emit("❌ [纯净能量] Stage2 失败，切换地图", "WARN")
            self._is_in_battle = False
            self._is_scanning_steady_state = True
            return self._neighbor_switch_twomap_after_fail(
                pem=True,
        ...
        n = self.PEM_SKILL4_MAINTENANCE_EVERY_BATTLES
        if self._battle_count % n == 0:
            self._emit(f"🔄 [纯净能量] 已满{n}场战胜，下场战斗前恢复精灵一", "INFO")
            self._pem_recover_pet_one_maintenance(use_foreground, stop_event)
    ...
    def _hit_target_mp3(lines: List[str], mp3_id_or_ids) -> bool:
        """检测是否命中目标 mp3（与内核 path= 分段一致，支持 / 与 \\ 及 pet\\sound\\ 等形式）。"""
    ...
    def _csv_log_battle(self, result: str, enemy_pet_ids=None):
        ...
        try:
            ...
            # ✅ 双塔：记录“上次战斗结束”的时间戳，用于判断 30min 无遇敌时重连前清缓存/tmp
    ...
    def run_rotation_mode(
        ...
        rotation_rare_slot: str = "shuangta",  # 非尼奥时段：任意稀有 profile slug
    ) -> None:
        """
        ...
        self._emit(
            f"🔄 [轮换模式] 非尼奥稀有：{prof.name}（slug={slot}）",
            "INFO",
        )
        # ✅ 设置轮换模式标志
        self._is_rotation_mode = True
        ...
        try:
            # ✅ Dashboard 点击启动后、首轮回环前：按当前日历段先对齐一次 SWF（与循环内 Step1 前对齐一致）
            _cm_start, _ = self._detect_rotation_mode(quiet=True)
            self._sync_swf_for_rotation_segment(
                _cm_start, log_tag="轮换·启动后首次SWF对齐",
            )
            # ✅ 无限循环，确保轮换可以持续进行
            while not stop_event.is_set() and not getattr(self.bot, "stop_current", False):
                ...
                self._sync_swf_for_rotation_segment(
                    current_mode,
                    log_tag=f"轮换-本轮步骤1前·{current_mode}",
                )
                ...
                # 轮换子步骤1 = 全局 Step1+2（_rotation_step1_login）
                if stop_event.is_set():
                    break
                self._rotation_step1_login(
                    use_foreground,
                    stop_event,
                    force_molecule_converter=self._rotation_step1_force_molecule_converter(),
                )
                
                # 轮换子步骤2–4 = 全局 Step3 内子阶段
                if stop_event.is_set():
                    break
                if not self._rotation_clear_backpack_and_pick_or_skip(
                    current_mode, use_foreground, stop_event, log_tag="轮换模式"
                ):
                    if stop_event.is_set() or getattr(self.bot, "stop_current", False):
                        stop_event.set()
                        break
                ...
                if self._pending_rotation_switch:
                    # Dashboard「停止」常只置 stop_current、不置轮换层 stop_event；若误判待切换会先跑重连，
                    # 看起来像「停了却还在轮换重连」，须显式跳过。
                    user_requested_stop = stop_event.is_set() or getattr(
                        self.bot, "stop_current", False
                    )
                    if user_requested_stop:
                        self._emit(
                            "ℹ️ [轮换模式] 已请求停止，跳过本次模式切换重连（待切换标志已清除）",
                            "WARN",
                        )
            ...
            if self._rotation_total_stats:
                ...
                # 非尼奥稀有（双塔/螳螂共用计数桶）
                shuangta_total = self._rotation_total_stats["shuangta"]
                self._emit("🌲 非尼奥稀有时段统计（累计）：", "SYSTEM")
    ...
    def _detect_rotation_mode(self, quiet: bool = False) -> Tuple[str, datetime]:
        ...
        quiet: True 时不刷 INFO 日志（供 _check_rotation_switch_time 高频一致性检测）
        """
        # ✅ 获取当前北京时间（永远以北京时间做基准）
    ...
    def _execute_rotation_reconnect(
        ...
        # ✅ 按「切入的下一段位」对齐 SWF（尼奥补齐；稀有补齐+删占位）。tmp 在 _rotation_step1_login 开头删除。
        self._sync_swf_for_rotation_segment(
            current_mode, log_tag=f"{reason}-模式切换对齐SWF",
        )
        ...
        try:
            ...
            # ✅ 步骤2+3：清空背包+取精灵（或内核判定三宠已就绪则跳过）
            if not self._rotation_clear_backpack_and_pick_or_skip(
                current_mode, use_foreground, stop_event, log_tag=reason
            ):
                # ✅ 步骤2失败，重新执行完整的1-5流程
    ...
    def _execute_flash_pipi_pre_rotation_reconnect(
        ...
        """
        闪光皮皮专用：与 `_execute_rotation_reconnect` 同构——子步骤1 = 全局 Step1+2，子步骤2–4 = 轮换 Step3–4（**Pick 波克尔/艾菲德斯/机塔**，与尼奥/野外统一），子步骤5 = `to闪光皮皮`+map 校验（不另走野外 `_execute_refresh_reconnect` 的 Step3 分发）。
        任一步失败返回 False，由调用方重试。
        """
        if self._rotation_reconnect_executing:
        ...
        reason = "闪光皮皮-轮换重连前置"
        self._rotation_reconnect_executing = True
        self._emit(f"🔄 [{reason}] 开始执行前置轮换重连（Pick：波克尔 / 艾菲德斯 / 机塔，与全模式统一）", "WARN")
        ...
        try:
            ...
            # 步骤2+3：清空背包+取精灵（或内核三宠就绪则跳过）
            if not self._rotation_clear_backpack_and_pick_or_skip(
                "shuangta", use_foreground, stop_event, log_tag=reason
            ):
    ...
    def _execute_mantis_pre_rotation_reconnect(
        ...
        """
        螳螂野外前置重连：与普通 ``run()`` 螳螂进图一致——
        「to螳螂」后执行 ``assets/regions/地图/10to11.json``（区域 ``地图.11``）进入 map11，
        再用 ``_wait_for_map_ready`` 等待 11.swf + newNpc（不在此用日志强验云上 map10）。
        """
        if self._rotation_reconnect_executing:
            self._emit("⚠️ [螳螂-轮换重连前置] 已在执行中，忽略本次请求", "WARN")
            return False

        reason = "螳螂-轮换重连前置"
        self._rotation_reconnect_executing = True
        self._emit(
            f"🔄 [{reason}] 开始执行前置轮换重连（Pick：波克尔 / 艾菲德斯 / 机塔；进 map11 步骤与野外一致）",
            "WARN",
        )
        ...
        try:
            ...
            self._emit(f"🔄 [{reason}-步骤5a] 执行 to螳螂.json", "SYSTEM")
            if hasattr(self.bot, "daily_runner"):
                self.bot.daily_runner.run_single_script("to螳螂", bg_mode=(not use_foreground))
            self._sleep_abortable(stop_event, 2.0)
            ...
            pf = DEFAULT_PROFILE_MANTIS
            self._emit(
                f"🗺️ [{reason}-步骤5b] 执行地图进入脚本 地图/"
                f"{_map_entry_script_stem(int(pf.map_swf_id))}.json（to螳螂 后直接进入 map11）",
                "SYSTEM",
            )
            if not self._execute_map_entry_script(pf.map_swf_id, use_foreground, stop_event):
                self._emit(f"⚠️ [{reason}] 地图进入脚本失败，返回False将由调用方重试完整流程", "WARN")
                return False

            self._emit(f"⏳ [{reason}-步骤5c] 等待进入 map {pf.map_swf_id}.swf + newNpc…", "SYSTEM")
            if not self._wait_for_map_ready(pf, stop_event):
                self._emit(
                    f"❌ [{reason}] 等待 map {pf.map_swf_id} 就绪超时或已停止",
                    "ERROR",
                )
                return False

            self._emit(f"✅ [{reason}] map {pf.map_swf_id} 就绪，可启动螳螂模式", "SUCCESS")
            return True
        except Exception as e:
            self._emit(f"❌ [{reason}] 执行异常: {e}", "ERROR")
            import traceback
    ...
    def _execute_nieo_pre_rotation_reconnect(
        ...
        """
        尼奥 / 纯净能量 **前置重连**：步骤1–4 与全模式对齐——背包清空后 **统一 Pick**
        波克尔 / 艾菲德斯 / 机塔（``_pickmode_rotation_step3_place_pets``；形参 ``"nieo"`` 仅为兼容）。
        旧称「尼奥三精灵（亨姆、波克尔、尼奥超能）」仅文案遗留，仓库点击已与野外稀有相同。

        步骤5：
        - ``pem_route=False``：``to尼奥`` + 校验 map11 + newNPC（再接 ``run_nieo_mode`` 进 10/11）。
        - ``pem_route=True``：``to纯净能量`` + 校验 map26 + newNPC（再接纯净能量首开进 27）。

        任一步失败时返回 False，由调用方重试完整流程。

        Returns:
            True: 成功完成前置准备
            False: 任一步失败，需重试完整流程
        """
        reason = "纯净能量模式-前置重连" if pem_route else "尼奥模式-前置重连"
        if self._rotation_reconnect_executing:
            self._emit(f"⚠️ [{reason}] 已在执行中，忽略本次请求", "WARN")
            return False
        ...
        if pem_route:
            self._current_mode = "pure_energy"
            self._emit(
                f"🔄 [{reason}] 开始执行前置重连（Pick：波克尔/艾菲德斯/机塔；步骤5：`to纯净能量` → map26）",
                "WARN",
            )
        else:
            self._emit(
                f"🔄 [{reason}] 开始执行前置重连（Pick：波克尔/艾菲德斯/机塔；步骤5：`to尼奥` → map11）",
                "WARN",
            )
        ...
        try:
            ...
            # 步骤2+3：清空背包+取精灵（或内核三宠就绪则跳过）
            if not self._rotation_clear_backpack_and_pick_or_skip(
                "nieo", use_foreground, stop_event, log_tag=reason
            ):
                self._emit(f"⚠️ [{reason}] 清空背包未完成，返回False将由调用方重试完整流程", "WARN")
                return False
            ...
            # 步骤5：`to尼奥`（map11）或 `to纯净能量`（map26）或自定义 to 脚本
            if to_script:
                script_tag = to_script
            else:
                script_tag = "to纯净能量" if pem_route else "to尼奥"
            self._emit(f"🔄 [{reason}-步骤5] 执行「{script_tag}」脚本", "SYSTEM")
            if hasattr(self.bot, "daily_runner"):
                script_name = script_tag
                self.bot.daily_runner.run_single_script(script_name, bg_mode=(not use_foreground))
            else:
                self._emit(f"⚠️ [{reason}-步骤5] bot.daily_runner 不存在", "WARN")
                return False
            ...
            if last_map_id == expected_map_id and has_newNPC:
                if to_script:
                    done_hint = f"可启动自定义尼奥（map A={expected_map_id}）"
                elif pem_route:
                    done_hint = "可启动纯净能量(资源)（首开 地图/26to27.json 进 27）"
                else:
                    done_hint = "可启动尼奥模式（进 10/11）"
                self._emit(
                    f"✅ [{reason}] 地图检查通过：map {last_map_id} + newNPC，{done_hint}",
                    "SUCCESS",
                )
            else:
                self._emit(
                    f"❌ [{reason}] 地图检查失败：map={last_map_id}（期望{expected_map_id}），"
                    f"has_newNPC={has_newNPC}，返回False将由调用方重试完整流程",
                    "ERROR",
                )
                return False

            self._emit(f"✅ [{reason}] 前置重连完成（含「{script_tag}」）", "SUCCESS")
            return True
        except Exception as e:
            self._emit(f"❌ [{reason}] 执行异常: {e}", "ERROR")
            import traceback
    ...
    def _rotation_step1_login(
        ...
        *,
        ...
        force_molecule_converter：传入 `_run_full_refresh_login_pipeline`；True 时基地门控后必跑分子转化仪（忽略 30min 节流）。
        skip_molecule_converter：True 时门控后直接跳过分子转化仪；**不写**节流时间戳，不影响其余模式计时。
        Returns:
            True：Step1+Step2 成功；False：被停止或失败（含重试耗尽前的早期失败路径）。
        """
        lp = "轮换模式-步骤1"
        self._emit(f"🔄 [{lp}] 开始登录流程", "SYSTEM")

        try:
            self._emit(f"🗑️ [{lp}] 轮换刷新：尝试删除游戏 tmp…", "INFO")
        ...
        if step1_res == "trinity_fail":
            ...
            time.sleep(2.0)
            if stop_event.is_set() or getattr(self.bot, "stop_current", False):
                self._emit(f"⛔ [{lp}] 重试前被停止", "WARN")
                return False
            return self._rotation_step1_login(
                use_foreground,
                stop_event,
                force_molecule_converter=force_molecule_converter,
                skip_molecule_converter=skip_molecule_converter,
        ...
        self._emit(
            f"🔄 [Step2·{lp}] 开始程序化登录（至基地/map{self.MAP_ID_AFTER_LOGIN} 门控）",
            "INFO",
        )
        if not self._run_full_refresh_login_pipeline(
            use_foreground,
            ...
            self._emit(f"⚠️ [{lp}] Step2 程序化登录/基地门控失败，重试", "WARN")
            if getattr(self.bot, "stop_current", False):
                return False
            return self._rotation_step1_login(
                use_foreground,
        ...
        self._mark_refresh_pipeline_map500001_gate_done()
        self._emit(
            f"✅ [{lp}] Step2 已完成，已 mark（与野外一致），继续轮换后续步骤",
            "INFO",
        )
        return True

    def run_pre_daily_mode(self, use_foreground: bool, stop_event: threading.Event) -> bool:
        """
        Dashboard「预选日常」：Step1（门控后不跑分子转化仪、不写节流戳）→ 清背包 →
        Pick 水8/草4/火4/地1 + 普通系双取（3与5），右翻均 15 → 仅开背包点身边跟随
        → `接受任务.json` → `每日签到.json`。
        """
        banner = "预选日常"
        task_accept_script = "接受任务"
        sign_in_script = "每日签到"
        self._emit(
            f"📋 [{banner}] 登录/基地门控（本趟跳过分子转化仪）→ 清背包 → Pick6 → "
            f"开背包直连身边跟随 → {task_accept_script} → {sign_in_script}",
            "SYSTEM",
        )
        if stop_event.is_set() or getattr(self.bot, "stop_current", False):
            return False
        if not self._rotation_step1_login(
            use_foreground,
            stop_event,
            force_molecule_converter=False,
            skip_molecule_converter=True,
        ):
            self._emit(f"❌ [{banner}] Step1/2（登录入门控）失败", "ERROR")
            return False
        if stop_event.is_set() or getattr(self.bot, "stop_current", False):
            return False
        if not self._rotation_step2_clear_backpack(use_foreground, stop_event):
            self._emit(f"❌ [{banner}] 清空背包失败或未就绪", "ERROR")
            return False
        if stop_event.is_set() or getattr(self.bot, "stop_current", False):
            return False
        ...
        rc_single = 15
        try:
            self._emit(f"📂 [{banner}-Pick] 打开精灵仓库…", "INFO")
            self._click_region("精灵仓库.打开", use_foreground)
            self._sleep_abortable(stop_event, 0.8)
        except Exception as e:
            self._emit(f"❌ [{banner}-Pick] 打开精灵仓库失败: {e}", "ERROR")
            return False

        self._rotation_place_pet_by_category(
            "水系", "水系倒数8", 8, rc_single, use_foreground, stop_event,
        )
        if stop_event.is_set() or getattr(self.bot, "stop_current", False):
            return False
        self._rotation_place_pet_by_category(
            "草系", "草系倒数4", 4, rc_single, use_foreground, stop_event,
        )
        if stop_event.is_set() or getattr(self.bot, "stop_current", False):
            return False
        self._rotation_place_pet_by_category(
            "火系", "火系倒数4", 4, rc_single, use_foreground, stop_event,
        )
        if stop_event.is_set() or getattr(self.bot, "stop_current", False):
            return False
        self._rotation_place_pet_by_category(
            "地面系", "地面倒数1", 1, rc_single, use_foreground, stop_event,
        )
        if stop_event.is_set() or getattr(self.bot, "stop_current", False):
            return False
        self._rotation_place_two_pets_same_category(
            "普通系",
            "普通倒数3",
            3,
            "普通倒数5",
        ...
        try:
            self._emit(f"📦 [{banner}] 关闭精灵仓库", "INFO")
            self._click_region("精灵仓库.关闭", use_foreground)
            self._sleep_abortable(stop_event, 0.5)
        except Exception as e:
            self._emit(f"⚠️ [{banner}] 关闭精灵仓库异常: {e}", "WARN")

        self._emit(f"🔄 [{banner}] 打开背包后直接身边跟随（无技能四判定）", "INFO")
        bag_open_key = "精灵背包.打开精灵背包"
        bag_open_btn_key = "精灵背包.打开精灵背包按钮"
        try:
            self._click_region(bag_open_btn_key, use_foreground)
        ...
        try:
            ...
            self._emit(f"❌ [{banner}] 点击身边跟随失败: {e}", "ERROR")
            return False

        dr = getattr(self.bot, "daily_runner", None)
        if dr is None:
            self._emit(f"❌ [{banner}] bot.daily_runner 不存在", "ERROR")
            return False
        bg_mode = not use_foreground

        self._emit(f"📜 [{banner}] 执行 {task_accept_script}.json", "SYSTEM")
        try:
            ok_accept = bool(dr.run_single_script(task_accept_script, bg_mode=bg_mode))
        except Exception as e:
            self._emit(f"❌ [{banner}] {task_accept_script} 异常: {e}", "ERROR")
            return False
        if stop_event.is_set() or getattr(self.bot, "stop_current", False):
            return False
        if not ok_accept:
            self._emit(f"⚠️ [{banner}] {task_accept_script} 脚本返回失败", "WARN")
            return False

        self._emit(f"📜 [{banner}] 执行 {sign_in_script}.json", "SYSTEM")
        try:
            ok = bool(dr.run_single_script(sign_in_script, bg_mode=bg_mode))
        except Exception as e:
            self._emit(f"❌ [{banner}] {sign_in_script} 异常: {e}", "ERROR")
            return False
        if ok:
            self._emit(f"✅ [{banner}] 完成（{task_accept_script} + {sign_in_script}）", "SUCCESS")
        else:
            self._emit(f"⚠️ [{banner}] {sign_in_script} 脚本返回失败或未完全成功", "WARN")
        return ok

    def _kernel_log_skip_rotation_pick_party_ready(self, log_tag: str) -> bool:
        """
        Step2 完成后：从 Step2 起点（或首次进图 map）起扫内核日志；
        最新两条 map 之间 pet/swf 恰为 {12,1337,1459} 时可跳过放回仓库+取精灵。

        不可用「屏蔽后 cursor」 alone：三宠 swf 多在进图 map 后、点屏蔽前就写入内核。
        """
        from core.kernel_log_match import (
            ROTATION_PARTY_FIGHT_PET_SWF_IDS,
        ...
        cursor = (
            getattr(self, "_refresh_pipeline_first_map_kernel_cursor", None)
            or getattr(self, "_refresh_pipeline_step2_kernel_cursor", None)
            or getattr(self, "_refresh_pipeline_shield_cursor", None)
        )
        if cursor is None:
            self._emit(f"ℹ️ [{log_tag}] 未记录 Step2/map 内核 cursor，不跳过放回仓库/取精灵", "INFO")
            return False

        seg = fetch_kernel_since(cursor)
        lines = seg if isinstance(seg, list) else []
        ids, bounds = collect_fight_pet_swf_ids_between_latest_two_maps(lines)
        if bounds is None:
            self._emit(
                f"ℹ️ [{log_tag}] Step2 日志不足两条 map，不跳过取精灵"
                f"（段内 {len(lines)} 行，已见 pet/swf={sorted(ids)}）",
                "INFO",
            )
            return False

        newer_idx, older_idx = bounds
        newer_map = first_map_id_in_line(lines[newer_idx])
        older_map = first_map_id_in_line(lines[older_idx])
        if ids == ROTATION_PARTY_FIGHT_PET_SWF_IDS:
            self._emit(
                f"✅ [{log_tag}] map {older_map}→{newer_map} 之间 pet/swf 恰为 {{12,1337,1459}}，"
                f"跳过放回仓库与取精灵，直接进入打开背包/跟随",
                "SUCCESS",
            )
            return True

        self._emit(
            f"ℹ️ [{log_tag}] map {older_map}→{newer_map} 之间 pet/swf={sorted(ids)} "
            f"≠ {{12,1337,1459}}，正常执行放回仓库与取精灵",
            "INFO",
        )
        return False
    ...
    def _rotation_clear_backpack_and_pick_or_skip(
        ...
        *,
        ...
        """
        Step3 轮换子阶段：若内核已加载三宠则跳过 step2+3；否则清空背包（含放回仓库）+ 取精灵。
        返回 False 表示 step2 失败，调用方应重连。
        """
        if self._kernel_log_skip_rotation_pick_party_ready(log_tag):
            return True
        if not self._rotation_step2_clear_backpack(use_foreground, stop_event):
            return False
        self._rotation_step3_place_pets(rotation_mode, use_foreground, stop_event)
        return True
    ...
    def _click_warehouse_jita_subtab(self, use_foreground: bool) -> None:
        """机塔取宠：点击「精灵仓库.机械龙系」页签。"""
        key = "精灵仓库.机械龙系"
        if not self.regions.get(key):
            raise KeyError(f"缺少区域：{key}（请用区域录制器标定机械龙系按钮）")
        self._click_region(key, use_foreground)
        self._emit(f"📂 [Pick-步骤3] 点击机械龙系（{key}）", "INFO")

    def _pickmode_rotation_step3_place_pets_yameisi(
        self, use_foreground: bool, stop_event: threading.Event
    ) -> None:
        """
        Pick 仓库 Step3（亚梅丝分支）：波克尔（飞行倒数1）
        + 同一超能系连续取 **亚梅丝（倒数3）** 与 **艾菲德斯（倒数20）**，
        不再走双属性/机械龙系机塔段。需 ``self._pickmode_yameisi_branch`` 为 True。
        """
        ...
        self._emit(
            "🔄 [Pick-步骤3·亚梅丝分支] 波克尔 + 超能系（亚梅丝倒数3、艾菲德斯倒数20）",
            "SYSTEM",
        )
        self._click_region("精灵仓库.打开", use_foreground)
        self._sleep_abortable(stop_event, 0.8)
        ...
        self._rotation_place_pet_by_category(
            "飞行系",
            "波克尔",
            _r("ROTATION_NIEO_REVERSE_FLIGHT", 1),
            _r("ROTATION_NIEO_RIGHT_CLICKS_FLIGHT", 25),
            use_foreground,
            stop_event,
        )
        self._rotation_place_two_pets_same_category(
            ...
            "艾菲德斯",
            20,
            _r("ROTATION_NIEO_RIGHT_CLICKS_PSYCHIC", 80),
            use_foreground,
            stop_event,
            reverse_order=False,
        )

        self._emit("📦 [Pick-步骤3·亚梅丝分支] 关闭精灵仓库", "INFO")
        self._click_region("精灵仓库.关闭", use_foreground)
        self._sleep_abortable(stop_event, 0.5)
    ...
    def _pickmode_rotation_step3_place_pets(
        ...
        """
        统一 Pick 仓库 Step3（尼奥 / 纯净能量 / 野外稀有 / 轮换前置等共用）：

        1. 飞行系倒数取 **波克尔**（右翻次数等仍读 ``config.ROTATION_NIEO_RIGHT_CLICKS_FLIGHT`` 等键名——历史遗留，与「仅尼奥使用」无关）；
        2. 超能系倒数第 **20** 取 **艾菲德斯**；
        3. **机塔**：双属性 → 机械龙系循环，探针「精灵仓库.1」非白后仅「放入背包」（见 ``PICKMODE_WAREHOUSE_*``）。

        若 ``self._pickmode_yameisi_branch``：同 1；超能系改为 **亚梅丝倒数3 + 艾菲德斯倒数20**（见 ``_pickmode_rotation_step3_place_pets_yameisi``），无机塔段。

        入战精灵 id：艾菲德斯 1459、机塔 1337；波克尔按对战内实际 id（探针/名单见 ``NIEO_ALLOWED_PET_IDS`` 等）。
        """
        if getattr(self, "_pickmode_yameisi_branch", False):
            self._pickmode_rotation_step3_place_pets_yameisi(use_foreground, stop_event)
            return
        ...
        self._emit("🔄 [Pick-步骤3] 打开精灵仓库并按 Pick 流程取宠", "SYSTEM")
        self._click_region("精灵仓库.打开", use_foreground)
        self._sleep_abortable(stop_event, 0.8)
        ...
        # 机塔：与「倒数扫格取第二个精灵」无关——无双击 1–9、无右翻；双属性→机械龙系（见 _click_warehouse_jita_subtab）
        jita_ok = False
        for attempt in range(40):
            ...
            try:
                ...
                self._emit("⚠️ [Pick-步骤3] 缺少区域 精灵仓库.双属性", "WARN")
                break
            self._sleep_abortable(stop_event, PICKMODE_WAREHOUSE_DUAL_ATTR_TO_MECHANICAL_SEC)
            try:
                self._click_warehouse_jita_subtab(use_foreground)
            except Exception as e:
                self._emit(f"⚠️ [Pick-步骤3] 无法点击机械龙系：{e}", "WARN")
                break
            ...
            if not self._warehouse_probe_is_mostly_white("精灵仓库.1"):
                ...
                break
            self._emit(
                f"🔄 [Pick-步骤3] 仅检测位置1：仍为白，重试双属性→机械龙系（{attempt + 1}/40）",
                "INFO",
            )

        if jita_ok:
            self._emit(
                "🎯 [Pick-步骤3] 机塔：位置1非白→仅点击放入背包（无格位双击/无右翻）",
                "INFO",
            )
            self._click_region("精灵仓库.放入背包", use_foreground)
            self._sleep_abortable(stop_event, 1.5)
            confirm_key = "对话框.普通确认"
            self._emit("✅ [Pick-步骤3] 快速双击普通确认（机塔）", "INFO")
            self._click_region_twice(confirm_key, use_foreground, gap=0.06)
            self._sleep_abortable(stop_event, 0.8)
        else:
            self._emit(
                "⚠️ [Pick-步骤3] 机塔：位置1持续为白，未执行放入背包",
                "WARN",
            )
    ...
    def _pickmode_rotation_step4_set_companion_yameisi(
        ...
        """Pick Step4（亚梅丝分支）：开背包→rare 判亚梅丝槽→双击→身边跟随；并写入 _yameisi_pos / _jita_pos（后者供出战优先序）。"""
        self._emit("🔄 [Pick-步骤4·亚梅丝分支] 设置身边跟随（技能四 rare 判亚梅丝）", "SYSTEM")
        bag_open_key = "精灵背包.打开精灵背包"
        bag_open_btn_key = "精灵背包.打开精灵背包按钮"
        try:
            self._click_region(bag_open_btn_key, use_foreground)
        ...
        slot = self._bag_select_slot_by_skill4_probe("yameisi", "rare", use_foreground)
        if slot:
            self._yameisi_pos = slot
            self._jita_pos = slot
        else:
            self._emit("⚠️ [Pick-步骤4·亚梅丝] 未匹配亚梅丝 rare 探针，默认双击精灵二", "WARN")
            self._yameisi_pos = "二"
            self._jita_pos = "二"

        self._click_bag_pet_slot_double(self._yameisi_pos or "二", use_foreground)
        self._sleep_abortable(stop_event, 1.5)

        self._emit("👥 [Pick-步骤4·亚梅丝] 点击身边跟随", "INFO")
        self._click_region("精灵背包.身边跟随", use_foreground)
        self._sleep_abortable(stop_event, 0.5)
        return True
    ...
    def _pickmode_rotation_step4_set_companion(
        ...
        """Pick Step4：开背包→技能四判机塔槽→双击→1.5s→身边跟随（不执行放回仓库）。判色同 `_pickmode_classify_skill4_rgb`。"""
        if getattr(self, "_pickmode_yameisi_branch", False):
            return self._pickmode_rotation_step4_set_companion_yameisi(
                use_foreground, stop_event
            )

        self._emit("🔄 [Pick-步骤4] 设置身边跟随（技能四双轴判机塔）", "SYSTEM")
        bag_open_key = "精灵背包.打开精灵背包"
        bag_open_btn_key = "精灵背包.打开精灵背包按钮"
        try:
            self._click_region(bag_open_btn_key, use_foreground)
        ...
        slot = self._bag_select_slot_by_skill4_probe("jita", "pickmode", use_foreground)
        if slot:
            self._jita_pos = slot
        else:
            self._emit("⚠️ [Pick-步骤4] 未匹配机塔探针，默认双击精灵二", "WARN")
            self._jita_pos = "二"

        self._click_bag_pet_slot_double(self._jita_pos or "二", use_foreground)
        self._sleep_abortable(stop_event, 1.5)

        self._emit("👥 [Pick-步骤4] 点击身边跟随", "INFO")
        self._click_region("精灵背包.身边跟随", use_foreground)
        self._sleep_abortable(stop_event, 0.5)
        return True
    ...
    def _rotation_step3_place_pets(self, current_mode: str, use_foreground: bool, stop_event: threading.Event) -> None:
        """
        轮换 / 重连 Step3：从精灵仓库取三只精灵放入背包。

        ``current_mode``（``\"nieo\"`` / ``\"shuangta\"`` 等）**已不参与分支**：自 Pick 统一后始终调用
        ``_pickmode_rotation_step3_place_pets``（波克尔 → 艾菲德斯 → 机塔）。保留形参仅为旧调用点兼容。
        """
        _ = current_mode
        self._pickmode_rotation_step3_place_pets(use_foreground, stop_event)
    ...
    def _rotation_step5_execute_to_script_and_start_mode(
        ...
        """
        第五步：执行 to 脚本、校验/进图，再开启对应模式。
        螳螂（to螳螂）：与前置重连一致，执行 地图/10to11 + _wait_for_map_ready。
        尼奥/双塔：仍用 _check_last_map_and_newnpc 校验期望 map。
        SWF：已在「本轮 Step1 前」「轮换切换重连入口」「轮换中标准刷新 Step3 前」与 calendar 段位对齐；此处不再重复。
        """
        ...
        if current_mode == "nieo":
            ...
            rs = getattr(self, "_rotation_rare_slot", "shuangta") or "shuangta"
            profile = self._resolve_rotation_rare_profile(rs)
            script_name = self._get_to_script_name(profile)
            if not script_name:
                self._emit(
                    f"❌ [轮换模式-步骤5] 无法解析 to 脚本：{profile.name}",
                    "ERROR",
                )
                return False
            expected_map_id = self._wild_rare_expected_map_zero_after_to(profile)
            if expected_map_id is None:
                expected_map_id = int(profile.map_swf_id)
        
        self._emit(f"📜 [轮换模式-步骤5] 执行{script_name}脚本", "INFO")
        if hasattr(self.bot, "daily_runner"):
            if current_mode != "nieo" and not self._to_script_has_steps(script_name):
                self._emit(
                    f"❌ [轮换模式-步骤5] {script_name}.json 无步骤，请用脚本录制器补录",
                    "ERROR",
                )
                return False
            self.bot.daily_runner.run_single_script(script_name, bg_mode=(not use_foreground))
        ...
        if current_mode == "nieo":
            self._emit(
                f"🗺️ [轮换模式-步骤5] 检查地图ID和newNPC（期望：map {expected_map_id} + newNPC）",
                "INFO",
            )
            last_map_id, has_newNPC = self._check_last_map_and_newnpc(expected_map_id, timeout_s=10.0)
            if last_map_id == expected_map_id and has_newNPC:
                self._emit(
                    f"✅ [轮换模式-步骤5] 地图检查通过：map {last_map_id} + newNPC",
                    "SUCCESS",
                )
            else:
                self._emit(
                    f"❌ [轮换模式-步骤5] 地图检查失败：检测到map={last_map_id}"
                    f"（期望：{expected_map_id}），has_newNPC={has_newNPC}，返回False，将由调用方重新执行完整流程",
                    "ERROR",
                )
                return False
        elif rs == "mantis":
            pf = profile
            self._emit(
                "🗺️ [轮换模式-步骤5] 螳螂：to螳螂 后直接执行地图进图脚本并等待 map11 + newNpc",
                "SYSTEM",
            )
            if not self._execute_map_entry_script(pf.map_swf_id, use_foreground, stop_event, profile=pf):
                self._emit(
                    "❌ [轮换模式-步骤5] 螳螂地图脚本执行失败，返回False将由调用方重新执行完整流程",
                    "ERROR",
                )
                return False
            if not self._wait_for_map_ready(pf, stop_event):
                self._emit(
                    "❌ [轮换模式-步骤5] 螳螂等待 map11 就绪超时或已停止，返回False将由调用方重新执行完整流程",
                    "ERROR",
                )
                return False
            self._emit(f"✅ [轮换模式-步骤5] 螳螂 map {pf.map_swf_id} 就绪", "SUCCESS")
        else:
            gate_z = wild_rare_map_gate_zero_literal_swf_ids(profile)
            anchor = int(expected_map_id)
            zero_banner = (
                "|".join(str(x) for x in sorted(gate_z)) + "（字面地图零）"
                if gate_z
                else str(anchor)
            )
            self._emit(
                f"🗺️ [轮换模式-步骤5] 检查地图零 map={zero_banner} + newNPC",
                "INFO",
            )
            last_map_id, has_newNPC = self._check_last_map_and_newnpc(
                anchor,
                timeout_s=10.0,
                accepted_map_ids=gate_z if gate_z else None,
            )
            if has_newNPC and self._kernel_map_matches_profile_expectation(
                profile, last_map_id, anchor
            ):
                self._emit(
                    f"✅ [轮换模式-步骤5] 地图零检查通过：map={last_map_id} + newNPC",
                    "SUCCESS",
                )
                self._on_map_zero_gate_passed()
            else:
                self._emit(
                    f"❌ [轮换模式-步骤5] 地图零检查失败：map={last_map_id}（期望 {zero_banner}），"
                    f"has_newNPC={has_newNPC}，返回False，将由调用方重新执行完整流程",
                    "ERROR",
                )
                return False
        ...
        if current_mode == "nieo":
            ...
            self._emit(
                f"🚀 [轮换模式-步骤5] 开启 {profile.name} 野外模式（非尼奥时段稀有）",
                "SYSTEM",
            )
            self.run(
                stop_event=stop_event,
    ...
    def _pinnacle_run_once(
        ...
        self._emit(
            "🖱️ [巅峰对战] 连点四下 刷新.开始 → 双击 刷新.登录 → 刷新登录循环(OCR选服)至 map",
            "INFO",
        )
        if not self._post_login_swf_programmatic_clicks(
    ...
    def _pinnacle_click_server_until_map(
        ...
        """
        兼容旧实现（已不再被巅峰流程调用）：
        统一委托到当前“刷新登录进图循环”逻辑，避免维护两套点服/进图代码。
        """
        return self._click_three_keys_until_map_refresh(
            use_foreground=use_foreground,
            stop_event=stop_event,
